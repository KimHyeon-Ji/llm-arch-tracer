...
## 순서: 함수를 먼저 읽는다, op 모양에서 추측하지 않는다

트레이스에 `slice`+`concat` 이 값을 반씩 나누는 모양으로 찍혀 있으면 "짝을 이루는 분할이겠구나"
같은 그럴듯한 설명이 먼저 떠오른다. **그 설명을 소스로 "확인"하지 말고, 그 op 를 실제로 만드는
함수 본문부터 통째로 읽고 나서 이름을 붙인다.** 순서가 바뀌면 위험하다 — 이미 세운 가설을
반박할 증거만 찾게 되고, 가설과 다른 설명이 소스에 있어도 못 알아본다.

실제 사례(DeepSeek-V4-Pro, 2026-08-31→09-01): Lightning Indexer 의 `slice→concat` 이 정확히
반씩(64+64) 나누는 것을 보고 "rotate_half 의 짝(even/odd 분할)"이라고 판정해 `c_I/2` 로
등록했다. 외부 검토(Codex)가 지적하고서야 `apply_rotary_pos_emb` 본문
(`nope = x[...,:-rope_dim], rope = x[...,-rope_dim:]`)을 처음부터 읽었는데, 실제로는
partial RoPE 의 **안 도는 부분(nope) / 도는 부분(rope) 경계**였다 — `c_I/2` 가 아니라
`c_I-d_rope` 와 `d_rope` 였다. 이 체크포인트에서 `c_I = 2 × d_rope` 라 값이 우연히 같았을
뿐이다. **op 모양이 아무리 "그 패턴처럼" 보여도, 그 op 를 실제로 만드는 함수(`apply_rotary_pos_emb`,
`naive_chunk_kda` 같은)를 먼저 열어서 정확히 뭘 하는지 확인한 뒤에 이름을 붙인다.**

특히 아래 경우엔 **op 모양만으로 판정하지 말 것**:
- 값이 **2개 이상**의 서로 다른 config 필드/심볼/유도식과 우연히 같다 (아래 「값이 겹치면」 참고)
- 압축·부분회전·게이팅처럼 "정확히 반으로 나뉜다"가 여러 가지로 설명될 수 있는 연산

이 원칙의 "결과물" 절반은 `develop/verify_all.py`가 자동으로 강제한다(`rules/references.yaml`
의 `value_collisions_reviewed` 항목이 실제 존재하는 파일:줄을 인용해야 함) — 하지만 **"추론
순서" 자체는 코드가 강제할 수 없다.** 실존하는 함수 이름을 인용하는 것과, 그 함수를 정말 트레이스
보기 전에 먼저 읽었는지는 다른 문제이고, 후자는 이 저장소의 어떤 게이트도 증명하지 못한다.
그래서 이 절은 여전히 사람(또는 LLM)이 스스로 지키는 습관으로 남는다.
