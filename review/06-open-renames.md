...
## 2. 왜 막히는가 — 이름이 **칸**에 붙어 있다 (진단 수정, 2026-08-14)

> **앞선 판(§2 아래)은 진단이 한 겹 얕았다.** "override 층이 모듈 단위라서"라고 적었는데,
> 그건 증상이다. 진짜 원인은 **이름의 단위가 텐서가 아니라 (op, 필드, 축) 칸**이라는 것이다.
> 한 텐서는 만든 op 의 `output_shape` 에 한 번, 그것을 쓰는 모든 op 의 `input_shape` 에 또
> 한 번씩 나타나는데 그 자리들이 서로 연결돼 있지 않다 — Qwen2.5-0.5B 하나만 봐도 텐서
> 2,673개에 라벨 자리 19,016개다. 그래서 한 자리를 고치면 옆자리가 옛 이름으로 남는다.
>
> **측정 (2026-08-14)**: 축 등가류를 보수적으로 계산했더니(`src/axis_classes.py`)
> 함대 전체 등가류 2,565,675개 중 **이름이 둘 이상인 것 6,033개(0.235%), 유형은 11가지**.
> 전부 이미 알려진 값 충돌이었다 — 등가류가 우리 판정과 독립적으로 같은 자리를 가리켰다.
>
> **그래서 필요한 것은 "전파"가 아니라 "한 번만 결정"이다.** 등가류마다 증거를 모아 이름을
> 한 번 정하고 모든 칸에 같은 값을 렌더하면, 데이터플로우 불일치는 검사 대상이 아니라
> **구조적으로 불가능**해지고, `label_overrides` 가 등가류를 고정하게 되어 아래 58건이
> 그대로 넣을 수 있게 된다.
>
> 트레이서는 트레이스 시점에 **진짜 텐서 신원을 갖고 있는데 행에 남기지 않는다**
> (`src/tracer.py:3` "depends_on is derived from tensor identity"). 남기면 shape 만으로
> 못 가르는 간선 15.9% 가 사라지지만 44개 모델 재추적이 필요하므로, 등가류 단위 결정을
> 먼저 하고 텐서 id 는 그 뒤에 측정해서 결정한다.

### 2-이전. (앞선 진단) 이름은 모듈이 아니라 텐서에 붙는다
...
- **RESOLVED (원인 확인, 2026-08-25)**: 처음엔 `develop/regen_summaries.py Kimi-K3`로 재확인했는데 두 분기 다 발동 안 하는 것처럼 보였다 — 원인은 shim이 아니라 **`regen_summaries.py`가 모델을 재실행하지 않는다는 것**이었다(그 파일 자신의 docstring: "Tracing is the expensive step and is intentionally skipped" — 기존 raw trace를 그대로 읽어 라벨링만 다시 한다). 그래서 `src/kda_shim.py`를 아무리 고쳐도 `regen_summaries.py`로는 절대 반영될 수 없었다.
  진짜 재트레이스(`src/run.py --profile develop/models/phase25-kimi-k3.yaml`)로 다시 확인하니 **둘 다 정확히 작동한다**: prefill의 `chunk_kda` 호출에 `safe_gate=True`가 들어오고 `naive_kda_lowerbound_gate`로 정확히 분기하며(Codex 제안대로 `sys.modules['fla.ops.kda'].chunk_kda`와 `modeling_kimi_linear.chunk_kda`의 `id()`가 완전히 동일함도 확인 — 캡처된 참조 문제 아님), decode의 conv1d 앞에 `concat`(캐시+새 토큰) op가 실제로 찍힌다. decode의 `fused_recurrent_kda`가 여전히 plain 공식을 쓰는 건 shim 문제가 아니라 **모델 자신의 소스가 그 호출부에 `safe_gate=`를 아예 안 넘기기 때문**(`lower_bound=`만 넘김, `modeling_kimi_linear.py:629-645`) — 정확한 재현이다.
- **남은 것 — 공식 산출물 미반영**: 이 검증은 스크래치 디렉터리 재트레이스로 한 것이고, `models/moonshotai__Kimi-K3/`는 아직 옛(수정 전) raw trace 그대로다. `develop/promote.py`로 승격하려면 `report.md`에 C-check FAIL이 없어야 하는데 지금 재트레이스에도 기존부터 있던 C10 FAIL(2개 파라미터 미사용)이 그대로 있어 승격 게이트를 못 넘는다. 게다가 decode에 `concat` op가 새로 끼면서 op_id가 밀리므로, 승격하려면 Kimi-K3 전용 위치 규칙·override·confirm을 전부 새 raw trace에 대고 재검증해야 한다 — 사실상 미니 재온보딩. 별도 작업으로 남겨둔다.
- **승격 완료 (2026-08-26)**: `src/run.py`에 A60(`_expert_cap_gap`)과 나란히 `_attn_res_layer0_gap` C10 예외를 추가한 뒤(af107c4e) `src/run.py --profile develop/models/phase25-kimi-k3.yaml --out develop/out`로 진짜 재트레이스, `develop/promote.py Kimi-K3`로 `models/`에 승격했다(b63771f6). 결과: **C10 FAIL 사라짐** — 남은 미커버 파라미터 전부가 문서화된 remedy(MoE 캡 셔플 + layer-0 attn_res gap)로 설명된다. 걱정했던 op_id 밀림 문제는 실제로는 재검증이 필요 없었다 — `label_confirmed.yaml`의 287개 확인 기록과 `canonical_axis_rules.yaml`의 KDA 규칙 7개가 전부 shape/scope/axis 기반 구조적 셀렉터라 op_id 자체를 안 쓴다. `develop/verify_all.py`: FAIL 4 → 3(C17/미해결상수/stub_ambiguous, 전부 A60과 같은 원인), 회귀 0건. 승격 과정에서 `promote.py`가 디렉터리를 통째로 교체하면서 `review_findings.json`/`.md`(src/run.py 산출물이 아니라 ④층이 수기로 만든 파일)가 지워질 뻔한 걸 커밋 전에 발견해 복구했다 — **다음에 promote.py를 쓸 때는 src/run.py가 만들지 않는 수기 파일이 그 모델 디렉터리에 있는지 먼저 확인할 것.**

### A62. moonshotai__Kimi-K3

- **모듈**: `model.layers.*.self_attn` (KDA 레이어)
- **축**: KDA 청크 스캔의 인트라-청크 순차 재귀 루프(`fla/ops/kda/naive.py:127-136` `for i in range(1, BT)`) 슬라이스 폭 중 값 8/12/16/48
- **지금 → 제안**: `2*d_conv`/`3*d_conv`/`4*d_conv`/`n_h_kda/2` → bare (이름 없음이 정답)  (확신 high — 근거는 이미 2026-08-25에 확정됨)
- **발견 경로**: ③ 자유 평가 재검토(2026-08-29, review_ledger가 이 모델을 '미수행'으로 보고해 처음부터 다시 봄). `models/moonshotai__Kimi-K3/review_findings.json`이 2026-08-25에 이미 이 정확한 4개 값을 `no_name_exists`로 판정해 뒀는데, `review_request.md`에 오늘도 그대로 다시 올라와 있어 **판정이 산출물에 한 번도 반영되지 않았다**는 것을 발견. `src/review_ledger.py`에도 이 2026-08-25 검토가 한 번도 기록(`--record`)되지 않아 게이트가 계속 "미수행"으로 봤다.
- **근거(오늘 독립 재확인)**: 실측 트레이스의 정확한 op_id를 추적함 — `model.layers.0.self_attn`의 `select`(op_id 2984/2991/2998, `[B,n_h_kda,5,d_chunk,d_chunk] -> [B,n_h_kda,5,d_chunk]`, `A[..., i, :]` 행 선택) → `slice`(op_id 2986/2993/..., `[..., d_chunk] -> [..., i]`, `A[..., i, :i]` 길이-i 절단)가 그대로 `naive_chunk_kda`의 `for i in range(1, BT): A[..., i, :i] = ...`(`.venv/Lib/site-packages/fla/ops/kda/naive.py:134-135`, `BT=chunk_size=64`)와 일치한다. 같은 사슬의 다른 i 값(예: i=7, op_id 2986)은 이미 정상적으로 bare `7`로 렌더되는데, 값이 다른 심볼과 우연히 같은 이 4개(8=2*d_conv, 12=3*d_conv, 16=4*d_conv, 48=n_h_kda/2, d_conv=4·n_h_kda=96)만 어떤 후속 휴리스틱 단계가 조작형 이름으로 다시 채운 것으로 보인다.
- **막힌 이유(측정)**: `rules/label_no_name.yaml`(A60이 만든 구조화된 no_name_exists 메커니즘)로 닫으려고 정확한 4개 selector를 등록했으나, `develop/verify_all.py`가 전부 `매치 0건 -- dead verdict`로 FAIL시켰다(8건, MoE 디스패치 쪽 위치 4개까지 같이 시도해서 총 8). 원인을 봤다: `src/axis_classes.py:bad_stub_count`가 읽는 `src/label_no_name.py:covered_keys`는 **`stub_ambiguous`로 표시된 unsettled 항목만** 매칭 대상으로 삼는다(A60 항목 자체가 정확히 이 경로를 위해 설계됨 — A60 절 참고). 이 4개 값은 ambiguous가 아니라(한 자리에 후보가 하나뿐) 애초에 `covered_keys()`의 대상이 아니고, 즉시 되돌렸다. **진짜 수정 경로는 `_unname_loop_indices`(`src/build_table.py`) 자체를 고쳐 이 4개 값이 다시 조작형 이름을 받지 않게 하는 것**인데, 이 함수는 오늘 이미 두 번의 서로 다른 정교화 시도(다중 피연산자 predecessor의 모호성 처리, 이어서 unary-only 보호 예외)가 전부 Qwen 함대에서 `heur` 퇴행으로 되돌아간 이력이 있다(git log 2026-08-29). 세 번째 시도는 이번엔 하지 않고 `open`으로 남긴다.
- **다음에 닫으려면**: (1) `_unname_loop_indices`를 다시 손대려면 Codex와 함께 "이미 bare로 렌더된 같은 사슬의 다른 값(i=7)은 왜 살아남는데 이 4개만 재작명되는가"부터 정확히 추적하는 게 먼저다(둘의 차이가 무엇인지 아직 모름 — 이번 세션은 원인을 재현할 시간까지는 못 냈다). (2) 또는 `bad_stub_count`/`covered_keys`에 non-ambiguous `no_name_exists` 판정도 인식하는 별도 경로를 새로 설계하는 방법도 있으나, 이것도 새 게이트 로직이라 이번엔 다루지 않는다(A60이 이미 같은 결론에 도달한 적 있음 — `bad_stub_count`에 `expected_gap` 예외를 새로 설계하는 문제, `review/dapper-mapping-crown` 계획 참고).
