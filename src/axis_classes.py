...
간선을 더 넣을 수 있는 자리(view 의 앞뒤 정렬, matmul 의 수축 축)는 지금 build_table 의
패스들이 사후에 하고 있는 일이다. 그것들을 여기 간선으로 옮기는 것이 다음 단계이며, 옮기고
나면 그 패스들은 "패치"가 아니라 "선언"이 된다.

**전치는 그 목록에서 뺐다 — 해 보고 물렸다 (2026-08-16).**

순열 인자는 트레이스에 없지만(`aten.transpose.int` 라는 이름뿐) 구체 크기로 역산할 수는
있다. 실측하니 28,139건 중 26,084건이 **유일하게** 역산되고 모호한 것은 0건이라, 간선을
놓아 봤다. 결과:

  * 통일 후 충돌 0, 퇴행 0 / 개선 3 -- 지표만 보면 괜찮았다.
  * 그런데 **소스로 확인한 판정 하나를 뒤집었다.** Zamba2 의 `repeat_kv` 뒤 key 축은
    소스상 `n_h` 인데(`num_attention_heads` 까지 반복된 뒤이므로) 등가류가 그 경계를 넘어
    `n_kv` 를 밀어 넣었다. 같은 연산 안에서 `[B,n_kv,T,d_head] -> [B,n_h,T,d_head/2]` 라는
    자기모순도 생겼다(op821).
  * 그 경계를 끊을 방법이 **없다**: Zamba2 는 n_h == n_kv == 32 라 `repeat_kv` 가
    `if n_rep == 1: return hidden_states` 로 빠져나가 **트레이스에 op 이 아예 없다.**

즉 전치 간선은 트레이스가 구별하지 못하는 것을 표가 단정하게 만든다. 이 자리를 열려면
먼저 **텐서 신원(또는 역할) 을 트레이스 시점에 남겨야 한다** -- 크기로 되짚는 것으로는
