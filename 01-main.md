...
> **`param_axes`는 아직 라벨링에 연결하지 않았다.** 계산해서 산출물에 싣기만 한다. 연결하면
> weight 축 라벨링(`relabel` 규칙 1)을 되살리는 셈인데, 오늘 한 번 실패한 자리다 — 다만 이번엔
> 축 역할이 곱셈 추측이 아니라 모듈 자신의 속성에서 오므로 근거가 다르다. 전수 대조 감사를
> 거친 뒤에 켠다.
