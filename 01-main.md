...
**실행 방법은 `/review-labels` 스킬이다**(`.claude/skills/review-labels/SKILL.md`) — 절차를 Markdown에 두고 세션의 WebFetch로 modeling 소스를 직접 읽으므로 **API 키도 SDK도 필요 없다**. 규칙을 YAML에, 절차를 MD에 두는 이 프로젝트의 방식과 같다. 무인 실행(CI·스케줄)이 필요할 때만 `src/review_agent.py`가 같은 일을 API로 하며, 둘은 같은 `review_findings.md`와 같은 원장에 쓴다.
