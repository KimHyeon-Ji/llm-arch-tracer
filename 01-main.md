...
**절차는 `review/` 폴더에 있다** — 특정 도구에 묶이지 않으므로 이 리포를 받은 사람은 어떤 환경에서도 같은 검토를 할 수 있다. 파이썬은 증거를 모아 `review_request.md`까지 쓰고 멈추고, `review/prompt.md`를 아무 LLM에나 넘기면 그 LLM이 캐시된 소스를 열어 판정한다. 결과는 `review_findings.json` 하나에 쓰고, 거기서 `review_findings.md`와 `model_summary.md`의 주의사항 절이 생성된다.
