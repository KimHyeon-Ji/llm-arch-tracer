...
**나머지 561건은 성격이 다르다.** 여기서부터는 유도 쪽이 틀린 경우가 섞인다 — DeepSeek-V3의 `mlp.gate`에서 유도가 만드는 `k*4*k`는 입력 축 라벨을 그대로 곱한 결과일 뿐 의미 있는 식이 아니다. 남은 덩어리는 MoE expert 레이아웃(`4*d_moe`/`E` vs `k*T`, 212건), Qwen3-Next의 linear attention(144건), DeepSeek-V4의 `n_h*d_v` vs `n_h*d_nope`(61건), Nemotron/Zamba2의 SSM 폭이며, 각각 해당 아키텍처를 확인해야 판정할 수 있다. 감사자는 어디를 볼지 알려줄 뿐 어느 쪽이 옳은지는 말해주지 않는다.
