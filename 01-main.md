...
`heur_*`는 이번 트레이스의 seq_len에서만 참일 수 있다. 2026-08-06 기준 함대 **0.61%**(등록 규칙 93.29%, 약한 근거 3.69%, 이름 없음 2.41%). 하루 만에 3.17% → 0.61%로 내려간 것은 두 가지 조치 덕이다:

1. **`T+1`을 규칙으로 등록.** decode의 KV 캐시 길이이고 트레이스가 그대로 보여준다 — `concat [B,n_kv,T,d_head] + [B,n_kv,1,d_head] -> [B,n_kv,T+1,d_head]`. prefill엔 0건, decode에만 800건. 값은 맞는데 근거가 산술이라 `heur_plus1`로 집계되던 36,813축이 규칙으로 옮겨갔다.
2. **반쪽 규칙에 `v >= 16` 바닥을 깔았다.** `+1` 규칙에는 이미 있던 가드다. Zamba2의 `d_conv`는 4라서 Mamba 블록의 모든 리터럴 2가 `d_conv/2`로 찍혔는데, 그 2는 `[B,1,...]` 두 개를 이어붙인 **피연산자 개수**이지 폭이 아니다. `d_conv/2`·`m_csa/2`·`d_conv_lin/2`·`k_grp/2` 4,468축이 정직한 정수로 돌아갔다. 진짜 반쪽인 `d_rope/2`(64)와 `d_head/2`(128·256·512)는 그대로다.

> 2번은 `bare`를 4,468 올린다. **여기서 `bare` 상승은 퇴행이 아니라 정직해진 것이다** — 지어낸 이름을 지운 자리이기 때문이다. 베이스라인은 이걸 자동으로 구분하지 못하므로 검토 후 수용했다.(gpt2-xl 9.03%가 최대, GPT-2가 `Conv1D`라 모듈 선언 폭이 없는 것과 같은 원인). 게이트가 `heur`를 베이스라인 지표로 추적하므로 "유도"를 "추측"으로 바꾸는 변경은 조용히 통과하지 못한다. `model_summary.md`는 지어낸 이름이 몰린 모듈을 함께 출력해 Tier 2 조사 대상을 바로 지목한다.

### 10.2 reshape 자기 유도 대조 (`reshape_incons`)

중간 텐서에는 물어볼 모듈이 없지만, **reshape는 자기 안에 증거를 갖고 있다** — `view [T,k] -> [T*k]`는 그 출력 축이 두 입력 축의 곱임을 값 매칭 없이 말해준다. `build_table.derive_from_reshape()`가 입력 축에서 출력 축을 독립적으로 유도하고 기존 라벨과 대조한다.

측정(26개 모델, 유도 가능한 축 239,056개): **97.6%가 기존 라벨과 일치**한다. 서로 다른 두 경로가 같은 답을 내므로 양쪽 모두에 대한 강한 증거다. 나머지가 볼 곳이다 — 순수 채움은 1,024축뿐이고, **불일치 2,258건(18개 모델)이 실제 가치**다.

불일치는 거의 전부 **값 충돌**이다. `d_model == n_h*d_head`인 모델(Qwen2.5-0.5B 896=14×64, SmolLM3-3B 2048=16×128 등)에서 q/k/v_proj 앞의 flatten이 **잔차 스트림을 패킹된 head 배치로 개명**한다. `self_attn` 스코프 안에서는 head 유도식이 평범한 심볼보다 우선하기 때문이고, 입력 쪽은 처음부터 맞게 붙어 있다.

**이 감사자가 두 개의 죽은 코드를 찾아냈다** (2026-08-05, 게이트 EXIT 0, 축 라벨 3,890건 교정, 전부 한 방향):

1. **bias가 앵커를 통째로 막고 있었다.** `_param_module`이 `len(params) != 1`이면 `None`을 반환했는데, `nn.Linear(..., bias=True)`는 op에 파라미터를 둘 넘긴다. 그래서 **bias가 있는 모든 투영 모듈**(Qwen2.5, SmolLM3, falcon, GPT-2 …)이 모듈 선언 폭 앵커를 하나도 못 받고 값 매칭 결과를 그대로 썼다. bias는 두 번째 폭이 아니라 `out_features`의 반복이므로 앵커를 거부할 근거가 없다 — `anchors.weight_param()`이 bias를 무시하고 weight만 고른다. 폭이 정말로 여럿인 모듈(gpt-oss `mlp.experts`)은 여전히 제외된다.
2. **`_upstream_module`이 항상 `None`을 반환하고 있었다.** 앵커를 모듈이 아닌 **파라미터** 기준으로 재키잉한 뒤(`...q_proj.weight`), 이 함수는 여전히 `module_key(module_path)`(`...q_proj`)로 조회했다. 키 형태가 달라 `key in dims`가 한 번도 참이 되지 않았고, 그래서 §6.3의 데이터플로우 교정이 조용히 아무것도 하지 않았다. 모듈 경로 → dims 키 매핑을 만들어 되살렸다.
3. bias가 있는 LayerNorm은 1-D 피연산자가 둘(weight·bias)이라 norm 앵커도 건너뛰어졌다. 둘의 폭은 같으므로 모호성이 아니다 — 후보들의 폭이 일치하면 채택한다. 이게 없으면 GPT-2의 `ln_2`가 앵커를 못 갖고 상류 탐색이 `attn.c_proj`까지 달려가 `mlp.c_fc`의 잔차 입력을 `n_h*d_head`로 만든다.

4. **활성 축 고정이 입력·출력을 구분하지 않았다.** 규칙 2는 `in_value`를 먼저 시도하고 `out_value`를 나중에 봤는데, `in_value == out_value`인 정사각 투영 — 즉 `d_model == n_h*d_head`인 모든 모델(Llama-3.1-70B 8192, 405B 16384, GPT-2, Qwen2.5) — 에서는 `in`이 항상 먼저 이겨서 **q_proj의 출력이 자기 입력 폭으로 찍혔다**. 지금은 렌더 중인 필드에 맞는 쪽을 먼저 본다(`output_shape`면 `out` 우선). 400축.

결과: `reshape_incons` **2,258 → 773 (−66%)**, `flow_ambig` 2,095 → 2,015, **26개 중 16개 모델이 0**이며 표준 attention 계열(Llama-3.1 8B/70B/405B, Llama-4, SmolLM3, Qwen2.5, OLMoE, gpt2-xl, falcon)은 전부 정리됐다. 라벨 변경 3,090건이 **예외 없이 `n_h*d_head`/`n_h*d_head/g_o`/`n_h*d_v` → `d_model`(또는 `d_attn`)** 방향이다. gpt2-xl의 `heur` +4는 어느 분기가 4개 축을 렌더하는지가 바뀐 계측상 부수효과로, 같은 모델에서 라벨 144건이 전부 교정된 것과 함께 검토 후 수용했다.

**MoE 라우팅 폭 212건은 진단이 끝났다 — 유도가 맞고 라벨이 틀렸다.** 증거는 op 자신에게 있다:

| 모델 | 트레이스 | 충돌 |
|---|---|---|
| GLM-4.5-Air | `view [T=16, k=8] -> [128]` | `E` 도 128 |
| DeepSeek-V4-Pro | `view [T=2048, k=6] -> [12288]` | `4*d_moe` 도 12288 |

`[T,k]`를 평탄화한 축은 정의상 라우팅된 토큰 슬롯 수(`k*T`)이지 expert 개수일 수 없다. gpt-oss가 같은 자리에서 `k*T`로 맞게 나오는 건 그 모델에서만 값이 안 겹치기 때문이다.

**해결됐다 (2026-08-06, 게이트 EXIT 0, 개선 13건·퇴행 0건).** `src/tdep.py`가 prefill과 decode의 구체 사이드카를 대조해 각 축이 시퀀스 길이에 따라 움직였는지 판정한다. `E`는 config 필드라 두 phase에서 같아야 하고 `k*T`는 달라야 하므로, 한 번의 비교가 결론을 낸다 — **추가 트레이싱 없이**, 이미 모든 모델 디렉터리에 있는 데이터로.

| 모델 | prefill | decode | 판정 |
|---|---:|---:|---|
| GLM-4.5-Air | 128 | 8 | 움직임 → `E`(128 고정) 배제 |
| DeepSeek-V4-Pro | 12288 | 6 | 움직임 → `4*d_moe`(12288 고정) 배제 |

증거 품질은 확실한 라벨로 검증했다: `d_model`·`d_head`·`n_h` 같은 config 축 291,142개 중 판정이 모순된 것은 **826개(0.3%)**뿐이고, 전부 prefill이 decode보다 scan 반복을 많이 도는 모델에서 op 순번이 어긋난 경우다.

**적용에는 세 겹의 절제가 필요했고, 셋 다 측정으로 정해졌다:**
1. **"안 움직였다"는 강제하지 않는다.** 원리적으로는 옳지만(고정 축은 T로 이름 붙일 수 없다), 대신 넣을 등록 이름이 없어 DeepSeek-V4-Flash 900축이 `bare`로 떨어졌다. 양성 판정 — 틀린 이름을 옳은 이름으로 **교체**하는 쪽 — 만 실행한다.
2. **모듈 커버리지 게이트.** 한 모듈의 prefill op가 **전부** decode 짝을 찾았을 때만 그 모듈의 판정을 쓴다. 부분 적용은 구간 일부만 개명해 한 텐서에 두 이름을 남기며, 이게 없을 때 Nemotron `flow_ambig` 0→210, V4-Pro 183→935였다.
3. **`k*T`를 등록 규칙으로 승격.** 안 하면 옳은 이름이 `heur_product`로 집계돼 GLM의 `heur`가 3,155→5,630으로 "퇴행"한다. 이건 지표가 정직해서 생기는 신호이지 품질 저하가 아니다.

덤으로 **퇴화된 곱 가드**가 필요했다: Llama-4 Maverick은 top-1 라우팅이라 `k*T`가 곧 `T`인데, 스코프 유도식이 평범한 심볼 `T`를 이겨 시퀀스 축 48개를 `k*T`로 만들었다. 인수 하나가 1인 곱은 별개 차원이 아니므로 규칙 자체를 건너뛴다. 같은 가드가 DeepSeek-V4-Pro의 `2*n_kv*d_head`(n_kv=1)도 걷어내 `d_g`가 제자리를 찾았다.

**결과: 축 라벨 10,272건 변경** — `4*d_moe→k*T` 3,599, `E→k*T` 2,475, `d_model*v_f→d_model` 2,560(xLSTM, v_f=1 퇴화), `2*n_kv*d_head→d_g` 1,473, `n_h→k*T` 165. `reshape_incons` 773→561, `flow_ambig` 2,015→1,826, `heur` **119,844→76,772**(대부분 `k*T`가 휴리스틱에서 등록 규칙으로 재분류된 것).

> **그전에 전파로 고치려던 시도는 두 번 다 되돌렸다**(같은 날). 유도 라벨을 권위로 삼아 전파하면 `reshape_incons` 773 → 8까지 가지만 `flow_ambig`가 2,015 → 2,187로 오른다. MoE 라우팅 구간에는 **텐서 정체가 실제로 바뀌는 op**(`sort`가 새 텐서를 내고 `index`가 모은다)가 있어 값 매칭이 그것들을 독립적으로 다시 이름 붙이기 때문이다. 전파를 같은-shape 출력까지 넓혀도 경계가 이동할 뿐이었다. **부분적으로 교정된 구간은 일관되게 틀린 구간보다 읽기 나쁘다** — 독자가 `k*T`와 `E` 중 어느 쪽이 맞는지 판단할 수 없다.
>
> 소스에서 고쳐야 하는데 지금은 등록 규칙으로 이길 수 없다: `E`의 `scope`가 `expert|moe|router|...`라 `mlp.experts`에서 **tier 1**(스코프 심볼)이고, 유도식은 tier 2다. 남은 선택지는 (a) `dim()`의 tier 순서를 손보는 전역 변경, 또는 (b) **prefill과 decode의 구체값을 비교해 축의 T 의존성을 판정**하는 것 — `E`는 두 phase에서 같고 `k*T`는 다르므로, 사이드카에 이미 있는 데이터로 결정 가능하다. (b)가 값 충돌 전반에 쓸 수 있는 일반해다.

**감사자 자체도 두 번 손봤다** (2026-08-06). 유도가 만든 식 중 신뢰할 수 없는 것을 보고에서 뺐다: ① 같은 심볼이 두 번 곱해진 것(`k*4*k` — 차원은 한 심볼의 제곱이 아니다), ② 선두 계수가 아닌 자리에 이름 없는 정수가 온 것(`n_g_ssm*5` — 심볼라이저가 이미 쓰는 "이름 없는 리터럴이 든 식은 진다" 규칙), ③ **등록된 이름과 그 정의식**(`d_inner` vs `d_head_ssm*n_h_ssm`은 같은 양이지 모순이 아니다). 561 → **348**. ③은 `E` vs `k*T`를 삼키지 않는다 — `k*T`는 `E`의 인수분해가 아니라 값만 같은 다른 양이고, 그렇게 말하는 규칙도 없다.

> **이 검사는 "둘 다 맞을 수는 없다"만 말하고 "어느 쪽이 틀렸다"는 말하지 않는다.** DeepSeek MLA가 그 예다: `view [B,T,n_h,d_nope] -> [B,T,n_h*d_v]`는 정확히 잡아내지만, 여기서는 **출력이 맞고 입력이 틀렸다** — attention 결과의 head 폭은 정의상 `d_v`인데 `d_nope`와 값이 같아(둘 다 128) 뺏겼다. 유도는 입력을 정답으로 가정하므로 제안하는 이름(`n_h*d_nope`)이 오히려 틀리다. 88건이 이 부류다.

**op 역할 규칙의 첫 조각이 들어갔다** (2026-08-06). `_split_from_authoritative`: reshape의 **출력 축이 권위 라벨**(모듈 선언에서 온 것)이고 그 값이 `A*B` 꼴이면, 입력에서 그 축을 만든 두 축의 이름이 곧 `A`와 `B`다. `anchors._ENABLE_SPLIT`이 못 했던 것을 여기서 하는 이유는 인수분해의 출처가 **같은 op 자신의 출력**이지 같은 블록의 아무 앵커가 아니기 때문이다. 권위 표시가 reshape를 넘어가도록 `_carry_authoritative`(권위 축만 나른다)와 번갈아 수렴시킨다.

효과: DeepSeek-V4의 압축기 축 순서가 바로잡혔다. **V4-Pro가 `[B, T/m_hca, d_head]` = `[1,16,512]`, V4-Flash가 `[B, T/m_csa, d_head]` = `[1,258,512]`**로 두 모델이 일치한다(예전엔 Pro만 뒤집혀 있었다). `ident_incons`는 **26/26 모델이 0** — 복사 op가 축의 의미를 바꾸는 모순이 함대에서 사라졌다.

**최종 288건**(17/26 모델이 0)은 전부 값이 겹치는 축의 **방향 판정** 문제이고, phase 증거로도 못 가린다(양쪽 다 T와 무관하므로). DeepSeek `d_v`/`d_nope` 94, Qwen3-Next linear attention 144, xLSTM `multihead_norm` 32, DeepSeek-V4 `d_head`/`T/m_csa` 60. 이걸 풀려면 값이나 스코프가 아니라 **op의 역할**(이 텐서는 attention 코어의 출력인가, Q/K의 nope 부분인가)을 아는 규칙이 필요하다 — `rules/structures/`의 확장 대상이다. 여기서부터는 유도 쪽이 틀린 경우가 섞인다 — DeepSeek-V3의 `mlp.gate`에서 유도가 만드는 `k*4*k`는 입력 축 라벨을 그대로 곱한 결과일 뿐 의미 있는 식이 아니다. 남은 덩어리는 MoE expert 레이아웃(`4*d_moe`/`E` vs `k*T`, 212건), Qwen3-Next의 linear attention(144건), DeepSeek-V4의 `n_h*d_v` vs `n_h*d_nope`(61건), Nemotron/Zamba2의 SSM 폭이며, 각각 해당 아키텍처를 확인해야 판정할 수 있다. 감사자는 어디를 볼지 알려줄 뿐 어느 쪽이 옳은지는 말해주지 않는다.

> **유도를 라벨러로 쓰는 것은 시도했다가 되돌렸다**(2026-08-05, 게이트가 잡음). view의 출력만 고치면 그 텐서를 받는 소비자는 옛 이름이라 `flow_ambig`가 함대 전체에서 두 배가 된다(Llama-3.1-70B 160→400, 405B 504→882). `_propagate_labels`는 설계상 단조라 이를 화해시키지 않는다. 오라벨은 reshape에 국한된 게 아니라 **그 스코프의 모든 op**에 걸리므로, 고칠 자리는 후처리가 아니라 우선순위 결정이다. 함수는 `_carry_reshape_labels`로 남겨두되 호출하지 않으며, 감사자로만 쓴다.
...
### 10.4 ③ 자유 평가는 수행 여부가 기록된다 (`review_ledger.yaml`)

리뷰 **패킷**(`full/review.md`)은 매 실행마다 자동 생성되지만, 그걸 읽고 판단하는 일은 자동이 아니다. 2026-08-06까지는 그 사실이 산출물에 드러나지 않았다 — 한 번도 검토되지 않은 모델과 오늘 아침 검토된 모델이 구별되지 않았고, 라벨을 바꾼 뒤에도 예전 검토가 유효해 보였다. C17이 온보딩에 대해 하는 일을 ③에도 한다:

**절차는 `04-label-review.md`에 있다** — 도구에 묶이지 않은 프로젝트 문서이므로 이 리포를 받은 사람은 어떤 환경에서도 같은 검토를 할 수 있다. Claude Code 사용자는 `/review-labels` 스킬이 그 문서를 그대로 실행하고(세션의 WebFetch로 modeling 소스를 읽으므로 **API 키도 SDK도 필요 없다**), 무인 실행(CI·스케줄)이 필요할 때만 `src/review_agent.py`가 같은 일을 API로 한다. 셋 다 같은 `review_findings.md`와 같은 원장에 쓴다.

`src/review_ledger.py`가 검토 대상 산출물(두 phase의 raw 트레이스 + `structure.yaml`)의 해시를 기록한다. 지금 해시와 다르면 그 검토는 **만료**다. 게이트가 `최신 / 만료 / 미수행`을 매번 보고한다.

**첫 수행(2026-08-06)에서 새 불변식이 나왔다 — `matmul_compose`.** 행렬곱은 정의상 `[m,k] @ [k,n] -> [m,n]`이므로 **수축 축의 이름이 같아야 하고 출력이 m·n이어야 한다.** 기존 검사는 전부 shape 하나씩만 보므로, 가중치 피연산자가 자체적으로는 그럴듯하면서 자기가 참여하는 곱과 모순될 수 있었다. 행렬곱 21,806개를 검사해 2개 모델에서 잡혔다:

```
Qwen2.5-0.5B q_proj:  [T, d_model] @ [d_model, d_model] -> [T, n_h*d_head]
                                              ^^^^^^^ out_features 인데 n_h*d_head 여야 함
```

bias 없는 Llama-3.1-70B는 같은 정사각 투영인데 `[d_model, n_h*d_head]`로 맞게 나온다 — **bias 있는 경로에서만** 어긋나므로 원인은 그쪽이다(미해결, 조사 안건에 남음). 검사는 게이트에 상시 탑재됐다.
