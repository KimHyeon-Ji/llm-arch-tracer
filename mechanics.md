...
HuggingFace의 공개 LLM을 가중치 없이(`meta` device) 실제로 실행해서, 레이어마다 어떤 연산이 도는지와 각 텐서 shape이 서로 어떻게 의존하는지를 자동으로 뽑아내는 도구입니다.

- **입력**: HuggingFace 모델 ID (예: `moonshotai/Kimi-K3`)
- **처리**: forward 실행 **중**에 모든 저수준 연산을 실시간으로 가로채 기록 → 미리 정리해 둔 이름 붙이기 규칙을 코드가 하나씩 대조해 적용 → 자동 검사
- **출력**: `[B, n_h, T, d_head]`처럼 아키텍처 의미가 담긴 shape 표와 구조 요약

실행 순서는 Python 코드가 결정하고, 이름을 무엇으로 붙일지는 별도로 정리해 둔 규칙 목록이 정한다 — 방법론을 적어둔 문서는 참고용일 뿐 실행을 지시하지 않는다.
