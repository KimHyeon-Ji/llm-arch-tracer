...
- **처리**: forward 실행 **중**에 모든 저수준 연산을 실시간으로 가로채 기록 → `rules/*.yaml` 규칙으로 이름 붙이기 → 자동 검사
- **출력**: `[B, n_h, T, d_head]`처럼 아키텍처 의미가 담긴 shape 표(`*.csv`)와 구조 요약(`structure.yaml`)

실행 순서는 Python 코드가, 이름 붙이는 기준은 `rules/*.yaml`이 정한다 — `.md` 문서는 방법론 설명일 뿐 실행 스펙이 아니다.
