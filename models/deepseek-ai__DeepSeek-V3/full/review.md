...
shape 축 **231,636개**를 렌더하면서 어떤 근거로 이름을 붙였는지의 내역이다. 위쪽 네 줄은 `rules/`에 **등록된 규칙**이 답을 준 경우이고, `휴리스틱`으로 시작하는 줄은 등록된 규칙이 없어 **산술적으로 맞는 이름을 지어낸** 경우다. 후자는 이번 트레이스의 seq_len에서만 참일 수 있으므로 그대로 신뢰하면 안 되고, `02-new-module-handling.md` Tier 2로 확인해 규칙으로 승격시켜야 한다.

| 근거 | 축 수 | 비율 |
|---|---:|---:|
| 런타임 축 (B/T/1) | 74,058 | 31.97% |
| 스코프 없는 심볼 | 64,972 | 28.05% |
| 이 모듈 스코프의 심볼 | 57,087 | 24.65% |
| 이 모듈 스코프의 유도식 | 21,717 | 9.38% |
| 휴리스틱: 두 심볼의 곱 | 6,380 | 2.75% |
| 같은 shape에서 이미 쓴 심볼 재사용 | 2,440 | 1.05% |
| 휴리스틱: 심볼의 배수 | 2,264 | 0.98% |
| 휴리스틱: 심볼+1 | 1,952 | 0.84% |
| 휴리스틱: 심볼의 절반 | 522 | 0.23% |
| 이름 없음 (정수 유지) | 244 | 0.11% |

등록된 규칙 **217,834축**, 약한 근거 2,440축, 휴리스틱 **11,118축 (4.8%)**, 이름 없음 244축.
...
```
  ...
  model.layers.N.mlp.gate                            expand           [T,k,1] -> [T,k,4*k]
...
```
  ...
  model.layers.N.mlp.gate                            expand           [B,k,1] -> [B,k,4*k]
