...
- `2*d_conv_lin` in `model.layers.*.linear_attn (레이어 3개)` — heur_multiple, 84축
...
| `d_rope` |  | `model.layers.*.linear_attn`, `model.layers.*.self_attn`, `model.rotary_emb` | 22138 |
| `d_head_lin_k` | 128 | `model.layers.*.linear_attn`, `model.layers.*.linear_attn.norm` | 8568 |
...
| `d_head_lin_v` | 128 | `model.layers.*.linear_attn` | 936 |
...
| 모듈 | 정수 | 축 수 | 같은 값의 심볼 |
|---|---|---|---|
| `model.layers.*.linear_attn` | 64 | 3072 | — |
...
### C. 모듈이 내는 출력 shape 전부 (60개 모듈 / 572종)
