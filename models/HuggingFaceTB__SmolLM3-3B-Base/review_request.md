...
| 라벨 | 값 | 나타나는 모듈 | 축 수 |
|---|---|---|---|
| `B` |  | `model.layers.*.self_attn`, `model.layers.*.input_layernorm`, `model.layers.*.post_attention_layernorm`, `model.layers.*.self_attn.q_proj` 외 50개 | 12578 |
| `T` |  | `model.layers.*.self_attn`, `model.layers.*.input_layernorm`, `model.layers.*.post_attention_layernorm`, `model.layers.*.self_attn.q_proj` 외 50개 | 7826 |
| `d_model` | 2048 | `model.layers.*.input_layernorm`, `model.layers.*.post_attention_layernorm`, `model.layers.*.self_attn.q_proj`, `model.layers.*.self_attn.k_proj` 외 45개 | 6242 |
| `d_head` | 128 | `model.layers.*.self_attn`, `model.rotary_emb` | 5426 |
| `n_h` | 16 | `model.layers.*.self_attn` | 4248 |
| `n_kv` | 4 | `model.layers.*.self_attn` | 2664 |
| `d_ff` | 11008 | `model.layers.*.mlp.gate_proj`, `model.layers.*.mlp.up_proj`, `model.layers.*.mlp.down_proj`, `model.layers.*.mlp` 외 1개 | 2088 |
| `T+1` |  | `model.layers.*.self_attn`, `model` | 1779 |
| `n_h*d_head` |  | `model.layers.*.self_attn.q_proj`, `model.layers.*.self_attn.o_proj`, `model.layers.*.self_attn` | 1296 |
...
### C. 모듈이 내는 출력 shape 전부 (54개 모듈 / 205종)
