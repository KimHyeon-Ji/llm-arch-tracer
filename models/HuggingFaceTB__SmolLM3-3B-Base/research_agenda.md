# 조사 안건 — HuggingFaceTB/SmolLM3-3B-Base
...
|---|---|---|---|---:|
| `model.layers.*.self_attn.o_proj` | `n_h*d_head` | `[n_h*d_head, n_h*d_head]` | `[2048, 2048]` | 108 |
| `model.layers.*.self_attn.q_proj` | `n_h*d_head` | `[n_h*d_head, n_h*d_head]` | `[2048, 2048]` | 36 |

## 확인할 소스 (신뢰도 순 — 위에서 답이 나오면 아래는 생략)

1. 실행 중인 modeling 소스의 주석·변수명·docstring — https://github.com/huggingface/transformers/blob/main/src/transformers/models/smollm3/modeling_smollm3.py
2. 같은 저장소 config 클래스의 docstring — https://github.com/huggingface/transformers/blob/main/src/transformers/models/smollm3/configuration_smollm3.py
3. 독립 서빙 구현 (vLLM / SGLang / TensorRT-LLM) — https://github.com/vllm-project/vllm/tree/main/vllm/model_executor/models
4. 저장소 README / 공식 model card — https://huggingface.co/HuggingFaceTB/SmolLM3-3B-Base
