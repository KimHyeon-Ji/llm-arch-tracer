# 조사 안건 — deepseek-ai/DeepSeek-V4-Flash
...
| 성격 | 조치 | 해당 축 |
|---|---|---:|
| 값이 겹쳐 어느 쪽인지 미결 | modeling 소스를 읽어야 함 | 11,740 |
| 규칙이 없어 이름을 못 붙임 | 확인 후 규칙 등록 | 0 |
| 이름이 존재하지 않음 | 정수로 두는 것이 정직 | 2,206 |
| 정사각 투영 (알려진 패턴) | 조사 불필요 — 축 순서만의 문제 | 172 |
...
|---|---|---|---|---:|
| `model.layers.*.attn_hc` | `n_hc` | `[B, T, n_hc, n_hc]` | `[1, 1032, 4, 4]` | 5418 |
| `model.layers.*.ffn_hc` | `n_hc` | `[B, T, n_hc, n_hc]` | `[1, 1032, 4, 4]` | 5418 |
| `model.layers.*` | `n_hc` | `[B, T, n_hc, n_hc]` | `[1, 1032, 4, 4]` | 602 |
| `model.layers.*` | `n_hc` | `[T, n_hc, n_hc]` | `[1032, 4, 4]` | 172 |
| `model.layers.*.mlp.experts` | `d_model` | `[E, d_model, d_model]` | `[256, 4096, 4096]` | 129 |
| `model.layers.*.attn_hc` | `n_hc` | `[n_hc, n_hc]` | `[4, 4]` | 86 |
| `model.layers.*.ffn_hc` | `n_hc` | `[n_hc, n_hc]` | `[4, 4]` | 86 |
...
| 모듈 | 값 | 축 수 |
|---|---:|---:|
| `model.layers.*.self_attn` | 2 | 1290 |
| `model.layers.*.self_attn.compressor.indexer` | 2 | 420 |
| `model.layers.*.self_attn.compressor` | 2 | 410 |
| `model.layers.*.attn_hc` | 3 | 43 |
| `model.layers.*.ffn_hc` | 3 | 43 |
...
4. 저장소 README / 공식 model card — https://huggingface.co/deepseek-ai/DeepSeek-V4-Flash
