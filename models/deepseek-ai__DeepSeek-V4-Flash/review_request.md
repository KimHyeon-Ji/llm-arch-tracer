...
- `4*d_model` in `model.hc_head` — heur_multiple, 42축
- `4*d_model` in `model.hc_head.input_norm` — heur_multiple, 15축
- `w_local+1` in `model.layers.*.self_attn (레이어 2개)` — heur_plus1, 14축
