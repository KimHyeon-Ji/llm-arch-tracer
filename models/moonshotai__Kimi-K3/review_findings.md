...
위 항목과 같은 반복 계단(값 48도 1~63 범위 안). `n_h_kda`(=96)의 절반과 우연히 같아 heur_half 가 지어낸 이름이다 -- KDA의 head 개수를 반으로 나눌 아키텍처적 이유가 없다. develop/verify/references.yaml의 irreducible_literals 항목이 이미 이 값 범위 전체를 문서화해 뒀다. **재확인(2026-08-29)**: 위 2*d_conv류 항목과 정확히 같은 상태 -- 아직 미반영, 같은 이유로 label_no_name.yaml 시도 후 되돌림. review/06-open-renames.md A62. **해소(2026-09-20)**: 이 라벨들은 발행 산출물에서 사라졌다 -- prefill/decode 트레이스 전수에서 `d_head-d_rope`/`d_head/2`/`2*d_conv`/`3*d_conv`/`4*d_conv`/`n_h_kda/2` 각 0 건, 해당 값(8/10/12/16/37/48)은 self_attn 안에서 이름 없는 정수로 각 1,173 건 남아 있다(decode 에는 이 청크 루프 자체가 없어 0 건). `rules/label_overrides.yaml` 의 교정이 다시 발화하도록 nth 앵커를 고쳐 잡은 결과이며(Q/K 정규화 수정으로 op_type 별 서수가 +2 밀려 있었다), 전환 분류에 `fabrication_withdrawn 552` 로 잡힌다.

## 발견 7 — should_be_no_name (반영됨)
