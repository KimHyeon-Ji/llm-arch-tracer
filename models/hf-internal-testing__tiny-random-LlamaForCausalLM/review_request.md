...
- `d_head vs n_h vs n_kv` in `model.layers.*.self_attn` — 값 4 를 두고 후보가 3개, 368축
...
| 라벨 | 값 | 나타나는 모듈 | 축 수 |
|---|---|---|---|
| `B` |  | `model.layers.*.self_attn`, `model.rotary_emb`, `model`, `model.layers.*.input_layernorm` 외 16개 | 794 |
...
| `n_h*d_head` |  | `model.layers.*.self_attn.q_proj`, `model.layers.*.self_attn.o_proj`, `model.layers.*.self_attn` | 72 |
| `n_kv*d_head` |  | `model.layers.*.self_attn.k_proj`, `model.layers.*.self_attn.v_proj`, `model.layers.*.self_attn` | 72 |
...
### C. 모듈이 내는 출력 shape 전부 (20개 모듈 / 133종)
