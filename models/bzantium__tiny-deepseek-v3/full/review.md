...
shape 축 **21,041개**를 렌더하면서 어떤 근거로 이름을 붙였는지의 내역이다. 위쪽 네 줄은 `rules/`에 **등록된 규칙**이 답을 준 경우이고, `휴리스틱`으로 시작하는 줄은 등록된 규칙이 없어 **산술적으로 맞는 이름을 지어낸** 경우다. 후자는 이번 트레이스의 seq_len에서만 참일 수 있으므로 그대로 신뢰하면 안 되고, `02-new-module-handling.md` Tier 2로 확인해 규칙으로 승격시켜야 한다.

| 근거 | 축 수 | 비율 |
|---|---:|---:|
| 런타임 축 (B/T/1) | 7,233 | 34.38% |
| 스코프 없는 심볼 | 5,847 | 27.79% |
| 이 모듈 스코프의 심볼 | 4,864 | 23.12% |
| 이 모듈 스코프의 유도식 | 2,192 | 10.42% |
| 스코프가 배제한 심볼 | 330 | 1.57% |
| 같은 shape에서 이미 쓴 심볼 재사용 | 240 | 1.14% |
| 휴리스틱: 심볼+1 | 192 | 0.91% |
| 휴리스틱: 심볼의 배수 | 119 | 0.57% |
| 이름 없음 (정수 유지) | 24 | 0.11% |

등록된 규칙 **20,136축**, 약한 근거 570축, 휴리스틱 **311축 (1.48%)**, 이름 없음 24축.
...
```
  ...
  model.layers.N.self_attn                           view             [B,T,d_head] -> [B,1,T,d_head]
  ...
  model.layers.N.self_attn                           slice            [B,1,T,d_head] -> [B,1,T,d_rope/2]
  ...
  model.layers.N.self_attn                           concat           [B,1,T,d_rope/2]*[B,1,T,d_rope/2] -> [B,1,T,d_head]
  ...
  model.layers.N.mlp.gate                            expand           [T,n_grp,1] -> [T,n_grp,2*n_grp]
  ...
  model.layers.N.mlp.gate                            sum              [T,E] -> [T,1]
  model.layers.N.mlp.gate                            elementwise_add  [T,1] -> [T,1]
  model.layers.N.mlp.gate                            div_             [T,E]*[T,1] -> [T,E]
...
```
  ...
  model.layers.N.self_attn.q_b_proj                  _unsafe_view     [B,(n_h+2*n_kv)*d_head] -> [B,1,(n_h+2*n_kv)*d_head]
  ...
  model.layers.N.self_attn                           split_with_sizes [B,1,c_kv+d_rope] -> [B,1,c_kv]*[B,1,d_head]
  ...
  model.layers.N.self_attn                           slice            [B,1,1,d_head] -> [B,1,1,d_rope/2]
  ...
  model.layers.N.self_attn                           concat           [B,1,1,d_rope/2]*[B,1,1,d_rope/2] -> [B,1,1,d_head]
  ...
  model.layers.N.mlp.gate                            expand           [B,n_grp,1] -> [B,n_grp,2*n_grp]
  ...
  model.layers.N.mlp.gate                            sum              [B,E] -> [B,1]
  model.layers.N.mlp.gate                            elementwise_add  [B,1] -> [B,1]
  model.layers.N.mlp.gate                            div_             [B,E]*[B,1] -> [B,E]
