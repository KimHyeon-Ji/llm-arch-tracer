# 검토 의뢰서 — microsoft/Phi-4

파이썬 파이프라인이 규칙으로 결정할 수 있는 것을 전부 결정하고, **판단이 필요한 것만** 여기 남겼다. 절차와 출력 형식은 `review/` 에 있다.

- transformers 모듈: `phi3`
- 판단 필요: **0건**

## 증거 — 이미 받아둔 실제 소스

- `develop/sources/modeling_phi3.py` — 있음, 이 파일을 열어서 판정한다
- `develop/sources/configuration_phi3.py` — 있음, 이 파일을 열어서 판정한다

- 온라인 원본: https://github.com/huggingface/transformers/tree/main/src/transformers/models/phi3
...
| prefill | `model.layers.*.self_attn.qkv_proj` | matmul | `[['T', 'd_model'], ['d_model', '(n_h+2*n_kv)*d_head']]` | `['(n_h+2*n_kv)*d_head', 'd_model']` | `[['T', '(n_h+2*n_kv)*d_head']]` |
...
| prefill | `model.layers.*.mlp.activation_fn` | silu | `[['B', 'T', 'd_ff']]` | `None` | `[['B', 'T', 'd_ff']]` |
...
| decode | `model.layers.*.self_attn.qkv_proj` | matmul | `[['B', 'd_model'], ['d_model', '(n_h+2*n_kv)*d_head']]` | `['(n_h+2*n_kv)*d_head', 'd_model']` | `[['B', '(n_h+2*n_kv)*d_head']]` |
...
| decode | `model.layers.*.mlp.activation_fn` | silu | `[['B', '1', 'd_ff']]` | `None` | `[['B', '1', 'd_ff']]` |
...
| 라벨 | 값 | 나타나는 모듈 | 축 수 |
|---|---|---|---|
| `B` |  | `model.layers.*.self_attn`, `model.layers.*.input_layernorm`, `model.layers.*.post_attention_layernorm`, `model.layers.*.mlp` 외 51개 | 15634 |
...
- `model.layers.*.mlp`
  ...
  - `[[B, T, d_ff], [B, T, d_ff]]`
  - `[[B, T, d_ff]]`
- `model.layers.*.mlp.activation_fn`
  - `[[B, 1, d_ff]]`
  - `[[B, T, d_ff]]`
