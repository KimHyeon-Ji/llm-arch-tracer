...
|---|---|---|---|---|
| `model.pos_emb` | rotary 축 (32) | `E` | `d_head/2` | 실측 `[1, 16, 32]` 이고 바로 옆이 `[1, 16, 64]`(=d_head) 다 — rotary 의 inv_freq 절반 축이다. 전문가 수 E(=32)와 값이 겹쳐 그 이름이 붙었다. `E` 는 `group: moe` 라 스코프 밖 폴백에서는 배제되므로 재사용·전파 경로로 들어온 것이고, 그 경로를 막는 건 값이 겹치는 축 전반에 영향을 준다. … |
...
```
  ...
  model.pos_emb                                      transpose        [B,32,T] -> [B,T,E]
  model.pos_emb                                      concat           [B,T,E]*[B,T,E] -> [B,T,d_head]
