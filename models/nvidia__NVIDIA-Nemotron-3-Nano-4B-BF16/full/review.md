...
shape 축 **81,354개**를 렌더하면서 어떤 근거로 이름을 붙였는지의 내역이다. 위쪽 네 줄은 `rules/`에 **등록된 규칙**이 답을 준 경우이고, `휴리스틱`으로 시작하는 줄은 등록된 규칙이 없어 **산술적으로 맞는 이름을 지어낸** 경우다. 후자는 이번 트레이스의 seq_len에서만 참일 수 있으므로 그대로 신뢰하면 안 되고, `02-new-module-handling.md` Tier 2로 확인해 규칙으로 승격시켜야 한다.
...
등록된 규칙 **75,478축**, 약한 근거 4,492축, 휴리스틱 **0축 (0.0%)**, 이름 없음 1,384축.
...
```
  ...
  model.layers.N.mixer                               constant_pad_nd  [B,T,n_h_ssm,d_head] -> [B,d_chunk,n_h_ssm,d_head]
  model.layers.N.mixer                               view             [B,d_chunk,n_h_ssm,d_head] -> [B,1,d_chunk,n_h_ssm,d_head]
  ...
  model.layers.N.mixer                               unsqueeze        [B,1,d_chunk,n_h_ssm,d_head] -> [B,1,d_chunk,1,n_h_ssm,d_head]
  model.layers.N.mixer                               unsqueeze        [B,1,d_chunk,n_h_ssm,d_head] -> [B,1,1,d_chunk,n_h_ssm,d_head]
  model.layers.N.mixer                               elementwise_mul  [B,1,d_chunk,1,n_h_ssm,d_head]*[B,1,1,d_chunk,n_h_ssm,d_head] -> [B,1,d_chunk,d_chunk,n_h_ssm,d_head]
  ...
  model.layers.N.mixer                               permute          [B,1,d_chunk,n_h_ssm,d_head] -> [B,1,n_h_ssm,d_chunk,d_head]
  model.layers.N.mixer                               permute          [B,1,d_chunk,n_h_ssm,d_head_ssm] -> [B,1,n_h_ssm,d_chunk,d_head_ssm]
  model.layers.N.mixer                               sum              [B,1,n_h_ssm,d_chunk,d_head,d_head_ssm] -> [B,1,n_h_ssm,d_head,d_head_ssm]
  model.layers.N.mixer                               permute          [B,1,n_h_ssm,d_head,d_head_ssm] -> [B,1,n_h_ssm,d_head_ssm,d_head]
  ...
  model.layers.N.mixer                               sum              [B,n_h_ssm,k,k,d_head_ssm,d_head] -> [B,n_h_ssm,k,d_head_ssm,d_head]
  ...
  model.layers.N.mixer                               sum              [B,1,d_chunk,n_h_ssm,d_head_ssm,d_head] -> [B,1,d_chunk,n_h_ssm,d_head_ssm]
