...
**부분 정정(Codex 외부 검토, 2026-08-30 같은 날 늦게)**: 59개 중 51개는 그대로 맞다 (Mamba/attention 레이어 스케줄과 대조해 교차 오염 없음을 재확인함). 하지만 8개는 앵커 자체가 잘못됐다 -- `[B,n_kv,n_h/n_g_ssm,T,d_head]`/`[B,n_kv,n_h/n_g_ssm,T+1,d_head]` (expand op, nth=0/1, axis1=n_kv·axis4=d_head 각 2×2=8개). 원인은 이 59건과 무관하게 **이전부터 있던** `derived_dims.yaml`의 `expr: "n_h // n_g_ssm", scope: "mixer|mamba|ssm"` 규칙이다 -- 이 스택은 attention/Mamba 블록이 전부 `mixer`라는 같은 이름을 쓰기 때문에, 이 유도식이 attention 행(원래 n_h/n_kv 자리)에도 새어 들어가 n_h/n_g_ssm 값이 우연히 일치하면 그쪽으로 잘못 렌더된다(fleet 전체로는 ~111행 영향, Nano decode에서는 d_conv+1이어야 할 Mamba 자리에도 새어듦). 8개를 `label_confirmed.yaml`에서 제거했다. **근본 수정은 보류**: derived formula에도 `layer_types`/`not_layer_types` 필터(현재 plain 심볼에만 있음)를 적용해야 하는데, 이는 새 인프라라 오늘 범위 밖으로 남겼다(Codex 권고).
