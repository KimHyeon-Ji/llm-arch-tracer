...
| 모듈 | 이전 | 이후 | 축 | 근거 |
|---|---|---|---|---|
| `mamba$` | `d_state` | `n_h_ssm` | 396 | transformers 5.14.1 installed source modeling_granitemoehybrid.py:209-714; revalidated this axis verdict unchanged. modeling_granitemoehybrid.py:403-414에서 B/C는 `[B,T,num_heads,state_size]`이고 sequence 축만 pad된다. 앞의 마지막 state 축 교정 이후 앵커에서 축 2는 n_h_ssm이다. |
...
| `mamba$` | `d_state` | `n_h_ssm` | 468 | transformers 5.14.1 installed source modeling_granitemoehybrid.py:209-714; revalidated this axis verdict unchanged. modeling_granitemoehybrid.py:645-648에서 dt는 `[B,self.num_heads,self.head_dim]`으로 expand된다. 축 1은 n_h_ssm이다. |
...
| `mamba$` | `d_state` | `n_h_ssm` | 576 | transformers 5.14.1 installed source modeling_granitemoehybrid.py:209-714; revalidated this axis verdict unchanged. modeling_granitemoehybrid.py:403-428에서 A는 chunk reshape 뒤 `permute(0,3,1,2)`를 거쳐 `[B,num_heads,n_chunks,chunk_size]`가 된다. 축 1은 d_state가 아니라 n_h_ssm이다. |
| `mamba$` | `d_state` | `n_h_ssm` | 468 | transformers 5.14.1 installed source modeling_granitemoehybrid.py:209-714; revalidated this axis verdict unchanged. modeling_granitemoehybrid.py:509,534,606-607에서 projected_states의 마지막 split 크기는 `self.num_heads`이고 그 출력이 dt다. shape_index 2의 마지막 축은 d_state가 아니라 n_h_ssm이다. |
...
```
  ...
  model.layers.N.mamba                               constant_pad_nd  [B,T,d_state,d_head_ssm] -> [B,d_chunk,n_h_ssm,d_head_ssm]
  model.layers.N.mamba                               elementwise_mul  [d_state,B]*[B,d_chunk,n_h_ssm,d_head_ssm] -> [B,d_chunk,n_h_ssm,d_head_ssm]
  model.layers.N.mamba                               unsqueeze        [B,T,n_h_ssm] -> [B,T,n_h_ssm,1]
  model.layers.N.mamba                               elementwise_mul  [B,T,d_state,d_head_ssm]*[B,T,n_h_ssm,1] -> [B,T,d_state,d_head_ssm]
  model.layers.N.mamba                               elementwise_mul  [d_state]*[B,T,n_h_ssm] -> [B,T,n_h_ssm]
  ...
  model.layers.N.mamba                               constant_pad_nd  [B,T,n_h_ssm] -> [B,d_chunk,d_state]
  ...
  model.layers.N.mamba                               permute          [B,1,d_chunk,d_state] -> [B,n_h_ssm,1,d_chunk]
  ...
  model.layers.N.mamba                               permute          [B,n_h_ssm,1,d_chunk,d_chunk] -> [B,1,d_chunk,d_chunk,d_state]
  ...
  model.layers.N.mamba                               sum              [B,1,d_chunk,d_chunk,d_state,d_head_ssm] -> [B,1,d_chunk,n_h_ssm,d_head_ssm]
  ...
  model.layers.N.mamba                               permute          [B,n_h_ssm,1,d_chunk] -> [B,1,d_chunk,d_state]
  ...
  model.layers.N.mamba                               select           [B,n_h_ssm,1,d_chunk] -> [B,d_state,1]
  ...
  model.layers.N.mamba                               transpose        [B,n_h_ssm,2,2] -> [B,2,2,d_state]
...
```
  ...
  model.layers.N.mamba                               expand           [B,1,1,d_state] -> [B,1,d_state,d_state]
