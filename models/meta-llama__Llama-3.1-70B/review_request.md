...
| 라벨 | 값 | 나타나는 모듈 | 축 수 |
|---|---|---|---|
| `B` |  | `model.layers.*.self_attn`, `model.layers.*.input_layernorm`, `model.layers.*.post_attention_layernorm`, `model.layers.*.self_attn.q_proj` 외 94개 | 29314 |
| `T` |  | `model.layers.*.self_attn`, `model.layers.*.input_layernorm`, `model.layers.*.post_attention_layernorm`, `model.layers.*.self_attn.q_proj` 외 94개 | 18086 |
| `d_model` | 8192 | `model.layers.*.input_layernorm`, `model.layers.*.post_attention_layernorm`, `model.layers.*.self_attn.q_proj`, `model.layers.*.self_attn.k_proj` 외 89개 | 13810 |
| `d_head` | 128 | `model.layers.*.self_attn`, `model.rotary_emb` | 13146 |
| `n_h` | 64 | `model.layers.*.self_attn` | 10080 |
| `n_kv` | 8 | `model.layers.*.self_attn` | 6560 |
| `d_ff` | 28672 | `model.layers.*.mlp.gate_proj`, `model.layers.*.mlp.up_proj`, `model.layers.*.mlp.down_proj`, `model.layers.*.mlp` 외 1개 | 4640 |
| `T+1` |  | `model.layers.*.self_attn`, `model` | 3935 |
| `n_h*d_head` |  | `model.layers.*.self_attn.q_proj`, `model.layers.*.self_attn.o_proj`, `model.layers.*.self_attn` | 2880 |
...
### C. 모듈이 내는 출력 shape 전부 (98개 모듈 / 293종)
