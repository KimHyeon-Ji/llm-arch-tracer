...
shape 축 **82,262개**를 렌더하면서 어떤 근거로 이름을 붙였는지의 내역이다. 위쪽 네 줄은 `rules/`에 **등록된 규칙**이 답을 준 경우이고, `휴리스틱`으로 시작하는 줄은 등록된 규칙이 없어 **산술적으로 맞는 이름을 지어낸** 경우다. 후자는 이번 트레이스의 seq_len에서만 참일 수 있으므로 그대로 신뢰하면 안 되고, `02-new-module-handling.md` Tier 2로 확인해 규칙으로 승격시켜야 한다.

| 근거 | 축 수 | 비율 |
|---|---:|---:|
| 런타임 축 (B/T/1) | 28,549 | 34.70% |
| 스코프 없는 심볼 | 20,988 | 25.51% |
| 이 모듈 스코프의 심볼 | 20,235 | 24.60% |
| 이 모듈 스코프의 유도식 | 10,058 | 12.23% |
| 같은 shape에서 이미 쓴 심볼 재사용 | 1,280 | 1.56% |
| 휴리스틱: 심볼+1 | 1,024 | 1.24% |
| 이름 없음 (정수 유지) | 128 | 0.16% |

등록된 규칙 **79,830축**, 약한 근거 1,280축, 휴리스틱 **1,024축 (1.24%)**, 이름 없음 128축.
...
```
  ...
  model.layers.N.self_attn.q_proj                    t                [n_h*d_head,n_h*d_head] -> w=[n_h*d_head,n_h*d_head] [n_h*d_head,n_h*d_head]
  model.layers.N.self_attn.q_proj                    view             [B,1,n_h*d_head] -> [B,n_h*d_head]
  ...
  model.layers.N.self_attn.k_proj                    t                [n_h*d_head,n_h*d_head] -> w=[n_h*d_head,n_h*d_head] [n_h*d_head,n_h*d_head]
  model.layers.N.self_attn.k_proj                    view             [B,1,n_h*d_head] -> [B,n_h*d_head]
  ...
  model.layers.N.self_attn.v_proj                    t                [n_h*d_head,n_h*d_head] -> w=[n_h*d_head,n_h*d_head] [n_h*d_head,n_h*d_head]
  model.layers.N.self_attn.v_proj                    view             [B,1,n_h*d_head] -> [B,n_h*d_head]
