...
**`STALE`** -- ③ 자유 평가 이후 산출물이 바뀜 (기록 bf5ba0662c448ddb != 현재 426f1d63dc887fc7, 검토일 2026-08-31)
...
| 모듈 | 무엇을 받아들였나 | 판정 | 근거 |
|---|---|---|---|
| `model.layers.*.mlp.experts` | expert GLU clamp | table_omits_computation | 외부 검토(Codex) 2026-09-16 지적에서 출발해 원인을 찾았다. gate 상한 7 / up [-7,7] clamp 가 요약 표에 없다. `clamp` 는 major-op 선별의 KEEP 목록에 없다 -- 비용을 지배하는 연산이 아니라는 판단이고 그 범위는 structure.yaml 이 밝힌다. 원시 trace 에는 있다(20b prefill op 198,199). modeling_gpt_oss.py |
