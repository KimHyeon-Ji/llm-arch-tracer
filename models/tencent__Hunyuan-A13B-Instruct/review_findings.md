...
**재확인(2026-08-30)**: `HunYuanMoEV1Config`(configuration_hunyuan_v1_moe.py, 108줄짜리 dataclass)가 선언한 필드는 `intermediate_size: int = 11008` 뿐이고 `moe_intermediate_size`는 여전히 없다. `src/source_check.check_aliases`가 인정하는 근거(config 클래스 선언 + base_fields() + `getattr(config, ...)` 패턴)에는 '체크포인트 JSON에 값이 우연히 일치하는 필드가 있다'는 화이트리스트가 없어서, 이 판정을 아무리 정확히 적어도 의뢰서 1절에는 구조적으로 계속 다시 올라온다 — Falcon-H1의 6절(정보성, 집행 메커니즘 없음)과 같은 부류. 판정 자체는 바뀔 이유가 없어 그대로 유지.
