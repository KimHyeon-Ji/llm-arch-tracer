...
Mamba2 의 청크 내 인과 마스크다 — `tril(ones(chunk_size, chunk_size))`, chunk_size=256. 정사각이 정상.
...
`modeling_zamba2.py:661` `self.intermediate_size = int(config.mamba_expand * self.hidden_size)` (2·2048=4096) 이고 `:699` 의 group_size 는 `intermediate_size // n_groups` 로 n_groups=1 이라 같은 값이 된다. 두 값이 완전히 겹쳐 트레이스만으로는 서로 못 가리므로 이번엔 등록하지 않았다.
