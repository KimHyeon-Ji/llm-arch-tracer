...
shape 축 **1,029,951개**를 렌더하면서 어떤 근거로 이름을 붙였는지의 내역이다. 위쪽 네 줄은 `rules/`에 **등록된 규칙**이 답을 준 경우이고, `휴리스틱`으로 시작하는 줄은 등록된 규칙이 없어 **산술적으로 맞는 이름을 지어낸** 경우다. 후자는 이번 트레이스의 seq_len에서만 참일 수 있으므로 그대로 신뢰하면 안 되고, `02-new-module-handling.md` Tier 2로 확인해 규칙으로 승격시켜야 한다.

| 근거 | 축 수 | 비율 |
|---|---:|---:|
| 런타임 축 (B/T/1) | 423,723 | 41.14% |
| 이 모듈 스코프의 심볼 | 256,216 | 24.88% |
| 스코프 없는 심볼 | 187,996 | 18.25% |
| 이 모듈 스코프의 유도식 | 78,851 | 7.66% |
| 같은 shape에서 이미 쓴 심볼 재사용 | 50,286 | 4.88% |
| 스코프가 배제한 심볼 | 12,515 | 1.22% |
| 휴리스틱: 심볼의 배수 | 9,738 | 0.95% |
| 이름 없음 (정수 유지) | 9,406 | 0.91% |
| 휴리스틱: 심볼의 절반 | 1,220 | 0.12% |

등록된 규칙 **946,786축**, 약한 근거 62,801축, 휴리스틱 **10,958축 (1.06%)**, 이름 없음 9,406축.
