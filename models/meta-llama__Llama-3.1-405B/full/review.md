...
| 이 모듈 스코프의 유도식 | 35,350 | 11.92% |
| 같은 shape에서 이미 쓴 심볼 재사용 | 5,040 | 1.70% |
| 이름 없음 (정수 유지) | 504 | 0.17% |

등록된 규칙 **291,082축**, 약한 근거 5,040축, 휴리스틱 **0축 (0.0%)**, 이름 없음 504축.
...
```
  ...
  model.layers.N.self_attn                           elementwise_mul  [B,n_kv,T,d_head]*[B,1,T,n_h] -> [B,n_kv,T,d_head]
  ...
  model.layers.N.self_attn.o_proj                    t                [d_model,n_h*d_head] -> w=[d_model,n_h*d_head] [d_model,n_h*d_head]
...
```
  ...
  model.layers.N.self_attn                           elementwise_mul  [B,n_kv,1,d_head]*[B,1,1,n_h] -> [B,n_kv,1,d_head]
