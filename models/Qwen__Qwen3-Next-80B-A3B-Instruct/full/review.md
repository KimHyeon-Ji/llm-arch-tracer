...
shape 축 **829,788개**를 렌더하면서 어떤 근거로 이름을 붙였는지의 내역이다. 위쪽 네 줄은 `rules/`에 **등록된 규칙**이 답을 준 경우이고, `휴리스틱`으로 시작하는 줄은 등록된 규칙이 없어 **산술적으로 맞는 이름을 지어낸** 경우다. 후자는 이번 트레이스의 seq_len에서만 참일 수 있으므로 그대로 신뢰하면 안 되고, `02-new-module-handling.md` Tier 2로 확인해 규칙으로 승격시켜야 한다.
...
등록된 규칙 **713,238축**, 약한 근거 3,792축, 휴리스틱 **14,106축 (1.7%)**, 이름 없음 98,652축.
...
```
  ...
  model.layers.N.linear_attn.conv1d                  conv1d           [B,4*d_model,T]*[4*d_model,B,d_conv_lin] -> w=[4*d_model,1,d_conv_lin] [B,4*d_model,n_h+2*n_kv]
  ...
  model.layers.N.linear_attn                         slice            [B,n_h_lin_v,1,d_rope] -> [B,n_h_lin_v,1,n_kv]
  model.layers.N.linear_attn                         unsqueeze        [B,n_h_lin_v,1,n_kv] -> [B,n_h_lin_v,1,n_kv,1]
  model.layers.N.linear_attn                         sum              [B,n_h_lin_v,1,n_kv,n_kv] -> [B,n_h_lin_v,1,n_kv]
  model.layers.N.linear_attn                         elementwise_add  [B,n_h_lin_v,1,n_kv]*[B,n_h_lin_v,1,n_kv] -> [B,n_h_lin_v,1,n_kv]
  model.layers.N.linear_attn                         copy_            [B,n_h_lin_v,1,n_kv]*[B,n_h_lin_v,1,n_kv] -> [B,n_h_lin_v,1,n_kv]
  ...
  model.layers.N.linear_attn                         sum              [B,n_h_lin_v,1,d_conv_lin,d_conv_lin] -> [B,n_h_lin_v,1,d_conv_lin]
  model.layers.N.linear_attn                         elementwise_add  [B,n_h_lin_v,1,d_conv_lin]*[B,n_h_lin_v,1,d_conv_lin] -> [B,n_h_lin_v,1,d_conv_lin]
  model.layers.N.linear_attn                         copy_            [B,n_h_lin_v,1,d_conv_lin]*[B,n_h_lin_v,1,d_conv_lin] -> [B,n_h_lin_v,1,d_conv_lin]
...
```
  ...
  model.layers.N.linear_attn.in_proj_ba              _unsafe_view     [B,d_rope] -> [B,1,d_rope]
  ...
  model.layers.N.linear_attn                         concat           [B,1,d_model]*[B,1,d_model]*[B,1,n_h*d_head] -> [B,1,4*d_model]
  model.layers.N.linear_attn                         transpose        [B,1,4*d_model] -> [B,4*d_model,1]
  model.layers.N.linear_attn                         squeeze          [4*d_model,B,d_conv_lin] -> w=[4*d_model,1,d_conv_lin] [4*d_model,d_conv_lin]
  model.layers.N.linear_attn                         concat           [B,4*d_model,d_conv_lin]*[B,4*d_model,1] -> [B,4*d_model,5]
  ...
  model.layers.N.linear_attn                         slice            [B,4*d_model,n_kv] -> [B,4*d_model,1]
  model.layers.N.linear_attn                         silu             [B,4*d_model,1] -> [B,4*d_model,1]
  model.layers.N.linear_attn                         transpose        [B,4*d_model,1] -> [B,1,4*d_model]
  model.layers.N.linear_attn                         split_with_sizes [B,1,4*d_model] -> [B,1,d_model]*[B,1,d_model]*[B,1,n_h*d_head]
  ...
  model.layers.N.self_attn                           view             [B,1,4*d_model] -> [B,1,n_h,n_kv*d_head]
  model.layers.N.self_attn                           split            [B,1,n_h,n_kv*d_head] -> [B,1,n_h,d_head]*[B,1,n_h,d_head]
