...
shape 축 **520,039개**를 렌더하면서 어떤 근거로 이름을 붙였는지의 내역이다. 위쪽 네 줄은 `rules/`에 **등록된 규칙**이 답을 준 경우이고, `휴리스틱`으로 시작하는 줄은 등록된 규칙이 없어 **산술적으로 맞는 이름을 지어낸** 경우다. 후자는 이번 트레이스의 seq_len에서만 참일 수 있으므로 그대로 신뢰하면 안 되고, `02-new-module-handling.md` Tier 2로 확인해 규칙으로 승격시켜야 한다.

| 근거 | 축 수 | 비율 |
|---|---:|---:|
| 런타임 축 (B/T/1) | 237,112 | 45.60% |
| 이 모듈 스코프의 심볼 | 146,306 | 28.13% |
| 이 모듈 스코프의 유도식 | 76,802 | 14.77% |
| 스코프가 배제한 심볼 | 39,200 | 7.54% |
| 스코프 없는 심볼 | 19,851 | 3.82% |
| 이름 없음 (정수 유지) | 768 | 0.15% |

등록된 규칙 **480,071축**, 약한 근거 39,200축, 휴리스틱 **0축 (0.0%)**, 이름 없음 768축.
...
```
  ...
  backbone.blocks.N.norm_mlstm                       mean             [B,T,d_model*v_f] -> [B,T,1]
  backbone.blocks.N.norm_mlstm                       elementwise_add  [B,T,1] -> [B,T,1]
  backbone.blocks.N.norm_mlstm                       rsqrt            [B,T,1] -> [B,T,1]
  backbone.blocks.N.norm_mlstm                       elementwise_mul  [B,T,d_model*v_f]*[B,T,1] -> [B,T,d_model*v_f]
...
```
  ...
  backbone                                           zeros            [] -> [B,n_h,d_model*qk_f/n_h]
  backbone                                           zeros            [] -> [B,n_h,1]
  backbone.blocks.N.norm_mlstm                       pow              [B,1,d_model*v_f] -> [B,1,d_model*v_f]
  backbone.blocks.N.norm_mlstm                       mean             [B,1,d_model*v_f] -> [B,1,1]
  backbone.blocks.N.norm_mlstm                       elementwise_add  [B,1,1] -> [B,1,1]
  backbone.blocks.N.norm_mlstm                       rsqrt            [B,1,1] -> [B,1,1]
  backbone.blocks.N.norm_mlstm                       elementwise_mul  [B,1,d_model*v_f]*[B,1,1] -> [B,1,d_model*v_f]
  backbone.blocks.N.norm_mlstm                       elementwise_mul  [B,1,d_model*v_f]*[d_model*v_f] -> [B,1,d_model*v_f]
  backbone.blocks.N.mlstm_layer.q                    t                [d_model*qk_f,d_model*v_f] -> w=[d_model*qk_f,d_model*v_f] [d_model*v_f,d_model*qk_f]
  backbone.blocks.N.mlstm_layer.q                    view             [B,1,d_model*v_f] -> [B,d_model*v_f]
  ...
  backbone.blocks.N.mlstm_layer.k                    t                [d_model*qk_f,d_model*v_f] -> w=[d_model*qk_f,d_model*v_f] [d_model*v_f,d_model*qk_f]
  backbone.blocks.N.mlstm_layer.k                    view             [B,1,d_model*v_f] -> [B,d_model*v_f]
  ...
  backbone.blocks.N.mlstm_layer.v                    t                [d_model*v_f,d_model*v_f] -> w=[d_model*v_f,d_model*v_f] [d_model*v_f,d_model*v_f]
  backbone.blocks.N.mlstm_layer.v                    view             [B,1,d_model*v_f] -> [B,d_model*v_f]
  backbone.blocks.N.mlstm_layer.v                    matmul           [B,d_model*v_f]*[d_model*v_f,d_model*v_f] -> w=[d_model*v_f,d_model*v_f] [B,d_model*v_f]
  backbone.blocks.N.mlstm_layer.v                    _unsafe_view     [B,d_model*v_f] -> [B,1,d_model*v_f]
  backbone.blocks.N.mlstm_layer.ogate_preact         t                [d_model*v_f,d_model*v_f] -> w=[d_model*v_f,d_model*v_f] [d_model*v_f,d_model*v_f]
  backbone.blocks.N.mlstm_layer.ogate_preact         view             [B,1,d_model*v_f] -> [B,d_model*v_f]
  ...
  backbone.blocks.N.mlstm_layer.ogate_preact         _unsafe_view     [B,d_model*v_f] -> [B,1,d_model*v_f]
  backbone.blocks.N.mlstm_layer.igate_preact         view             [B,1,d_model*v_f] -> [B,d_model*v_f]
  backbone.blocks.N.mlstm_layer.igate_preact         t                [n_h,d_model*v_f] -> w=[n_h,d_model*v_f] [d_model*v_f,n_h]
  ...
  backbone.blocks.N.mlstm_layer.fgate_preact         view             [B,1,d_model*v_f] -> [B,d_model*v_f]
  backbone.blocks.N.mlstm_layer.fgate_preact         t                [n_h,d_model*v_f] -> w=[n_h,d_model*v_f] [d_model*v_f,n_h]
  ...
  backbone.blocks.N.mlstm_layer                      view             [B,1,d_model*v_f] -> [B,1,n_h,d_head]
  ...
  backbone.blocks.N.mlstm_layer.multihead_norm       view             [B,1,n_h,d_head] -> [B,1,d_model*v_f]
  backbone.blocks.N.mlstm_layer.multihead_norm       elementwise_mul  [B,1,d_model*v_f]*[d_model*v_f] -> [B,1,d_model*v_f]
  backbone.blocks.N.mlstm_layer                      view             [B,1,d_model*v_f] -> [B,1,d_model*v_f]
  backbone.blocks.N.mlstm_layer.ogate_act_fn         sigmoid          [B,1,d_model*v_f] -> [B,1,d_model*v_f]
  backbone.blocks.N.mlstm_layer                      elementwise_mul  [B,1,d_model*v_f]*[B,1,d_model*v_f] -> [B,1,d_model*v_f]
  backbone.blocks.N.mlstm_layer.out_proj             t                [d_model*v_f,d_model*v_f] -> w=[d_model*v_f,d_model*v_f] [d_model*v_f,d_model*v_f]
  backbone.blocks.N.mlstm_layer.out_proj             view             [B,1,d_model*v_f] -> [B,d_model*v_f]
  ...
  backbone.blocks.N.mlstm_layer.out_proj             _unsafe_view     [B,d_model*v_f] -> [B,1,d_model*v_f]
