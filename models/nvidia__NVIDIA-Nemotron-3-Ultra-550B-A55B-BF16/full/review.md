...
| `mixer$` | `d_state` | `d_chunk` | 672 | transformers 5.14.1 installed source modeling_nemotron_h.py:72-572; revalidated this axis verdict unchanged. modeling_nemotron_h.py:278-290의 `A.permute(0,3,1,2)` 출력은 `[B,num_heads,n_chunks,chunk_size]`다. 따라서 마지막 128 축은 d_state가 아니라 d_chunk다. |
...
| `mixer$` | `d_state` | `d_chunk` | 96 | transformers 5.14.1 installed source modeling_nemotron_h.py:72-572; revalidated this axis verdict unchanged. modeling_nemotron_h.py:278-295에서 A는 `[B,num_heads,n_chunks,chunk_size]`로 재배열된다. singleton 출력의 축 3은 d_state가 아니라 d_chunk다. |
