...
shape 축 **121,989개**를 렌더하면서 어떤 근거로 이름을 붙였는지의 내역이다. 위쪽 네 줄은 `rules/`에 **등록된 규칙**이 답을 준 경우이고, `휴리스틱`으로 시작하는 줄은 등록된 규칙이 없어 **산술적으로 맞는 이름을 지어낸** 경우다. 후자는 이번 트레이스의 seq_len에서만 참일 수 있으므로 그대로 신뢰하면 안 되고, `02-new-module-handling.md` Tier 2로 확인해 규칙으로 승격시켜야 한다.

| 근거 | 축 수 | 비율 |
|---|---:|---:|
| 런타임 축 (B/T/1) | 35,464 | 29.07% |
| 이 모듈 스코프의 심볼 | 34,302 | 28.12% |
| 스코프 없는 심볼 | 32,672 | 26.78% |
| 이 모듈 스코프의 유도식 | 13,405 | 10.99% |
| 휴리스틱: 심볼+1 | 2,448 | 2.01% |
| 같은 shape에서 이미 쓴 심볼 재사용 | 1,920 | 1.57% |
| 이름 없음 (정수 유지) | 1,056 | 0.87% |
| 휴리스틱: 두 심볼의 곱 | 384 | 0.31% |
| 스코프가 배제한 심볼 | 338 | 0.28% |

등록된 규칙 **115,843축**, 약한 근거 2,258축, 휴리스틱 **2,832축 (2.32%)**, 이름 없음 1,056축.

지어낸 이름이 가장 많이 붙은 자리 (여기부터 확인하면 된다):

| 모듈 | 라벨 | 규칙 | 축 수 |
|---|---|---|---:|
| `model.layers.0.self_attn` | `T+1` | 휴리스틱: 심볼+1 | 52 |
| `model.layers.1.self_attn` | `T+1` | 휴리스틱: 심볼+1 | 52 |
| `model.layers.2.self_attn` | `T+1` | 휴리스틱: 심볼+1 | 52 |
| `model.layers.4.self_attn` | `T+1` | 휴리스틱: 심볼+1 | 52 |
| `model.layers.5.self_attn` | `T+1` | 휴리스틱: 심볼+1 | 52 |
| `model.layers.6.self_attn` | `T+1` | 휴리스틱: 심볼+1 | 52 |
| `model.layers.8.self_attn` | `T+1` | 휴리스틱: 심볼+1 | 52 |
| `model.layers.9.self_attn` | `T+1` | 휴리스틱: 심볼+1 | 52 |
| `model.layers.10.self_attn` | `T+1` | 휴리스틱: 심볼+1 | 52 |
| `model.layers.12.self_attn` | `T+1` | 휴리스틱: 심볼+1 | 52 |
| `model.layers.13.self_attn` | `T+1` | 휴리스틱: 심볼+1 | 52 |
| `model.layers.14.self_attn` | `T+1` | 휴리스틱: 심볼+1 | 52 |
