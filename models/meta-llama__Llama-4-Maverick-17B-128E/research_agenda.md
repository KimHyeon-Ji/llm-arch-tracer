...
| 규칙이 없어 이름을 못 붙임 | 확인 후 규칙 등록 | 48 |
| 이름이 존재하지 않음 | 정수로 두는 것이 정직 | 288 |
| 정사각 투영 (알려진 패턴) | 조사 불필요 — 축 순서만의 문제 | 192 |
...
|---|---|---|---|---:|
| `model.layers.*.self_attn.o_proj` | `n_h*d_head` | `[n_h*d_head, n_h*d_head]` | `[5120, 5120]` | 144 |
| `model.layers.*.self_attn.q_proj` | `n_h*d_head` | `[n_h*d_head, n_h*d_head]` | `[5120, 5120]` | 48 |

## 4. 미등록 config 필드 — 이름을 붙일 근거 자체가 없다

이 아키텍처가 실제로 쓰는 config 필드인데 `rules/symbols.yaml` 이 모른다. 역할을 확인해 `aliases` 또는 `derived_dims.yaml` 에 등록한다.
