...
지금은 **보고만** 한다. 이름을 등가류 단위로 한 번만 결정하는 것이 다음 단계이고, 그때 이 값이
0 이 되어야 한다 — 그래서 기준선에 박아 두었다. 자세히는 `review/06-open-renames.md`.
...
| 하드 불변식 위반 | **0** — 위 §3② 표의 16종 전부, prefill·decode 양쪽 |
| 게이트 | FAIL 0 · 셀프테스트 28/28 (죽은 검사 0) |
