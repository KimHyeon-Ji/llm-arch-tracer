...
models/Qwen__Qwen2.5-0.5B/
  ...
  full/
    ...
    *.axis_classes.json            축 등가류 감사 — 같은 축인데 이름이 둘인 자리
    *.unsettled.json               규칙이 끝내지 못해 ④층으로 넘긴 축 + override 초안
...
| **값싼 "확인 못함"** | HF 소스 밖을 찾아본 흔적(URL) 없이 `undetermined` 로 남겼다 |
| **한 축 한 이름** | 같은 축(등가류)인데 자리마다 다른 이름이 붙었다 |
| **근거 없는 확인** | "봤고 맞다"고 적어 그 축을 인계에서 뺐는데 무슨 소스를 읽었는지 안 적었다 |
| **낡은 확인** | 확인해 둔 이름이 그 뒤 바뀌었다 (`matched: 0`) |

그리고 **축 등가류 감사**(`src/axis_classes.py`, `full/*.axis_classes.json`)가 있다.

이름은 지금 **텐서가 아니라 칸에 붙는다** — 한 텐서는 만든 op 의 `output_shape` 에 한 번,
그것을 쓰는 모든 op 의 `input_shape` 에 또 한 번씩 나타나는데, 그 자리들이 서로 연결돼 있지
않고 각각 값 매칭으로 이름을 얻는다(Qwen2.5-0.5B: 텐서 2,673개 / 라벨 자리 19,016개).
그래서 소스로 확정한 이름을 한 자리에 넣으면 옆자리가 옛 이름으로 남는다 — 지금 막혀 있는
교정 58건이 전부 이 하나의 원인이다.

감사는 "어느 자리들이 같은 축인가"를 보수적으로 계산하고(모호하면 잇지 않는다), 한 등가류에
이름이 둘 이상인 곳을 센다. 함대 전체 **등가류 256만 중 충돌 6,033건(0.235%)** 이고,
그 충돌은 **11가지 유형**으로 전부 이미 알려진 값 충돌이다(`d_model` vs `n_h*d_head` 4,368건 등).

**2026-08-14 부터 이 값은 함대 전체에서 0 이다.** 감사에서 그치지 않고 이름을 실제로 등가류
단위로 결정하게 바꿨기 때문이다(`build_table._unify_axis_classes`). "같은 축"이라는 선언은 전부
간선이 되고 — 생산자→소비자, shape 보존 elementwise, reshape 오른쪽 정렬, 행렬곱 합성 —
**이름을 쓰는 곳은 통일 패스 하나뿐**이다. 승자는 그 축을 가장 먼저 만들어 낸 출력 자리다:
구조에서 나온 이름(합성·선언 폭·split 조각·전치 순열)이 값 매칭보다 낫기 때문이다.

효과: 등가류 충돌 6,033 → **0**, `flow_ambig` 5,568 → 732, `matmul_compose` 725 → 0.
그리고 소스 판정을 표에 넣는 층도 이제 등가류를 고정한다(`spread: class`) — 모듈 경계에서
멈춰 이웃이 옛 이름을 유지하던 것이 교정 58건을 막고 있던 유일한 원인이었다.
...
값으로 결정할 수 없는 축이 있다. 두 config 필드가 이 체크포인트에서 같은 값이면 값으로는
영원히 못 가르고, 정규식과 우선순위를 더 비트는 것은 **다른 모델을 깨뜨리는 길**이다 —
2026-08-13~14 에 그 길로 세 번 갔다가 세 번 되돌렸다.

그래서 그런 축은 규칙이 스스로 넘긴다(`full/*.unsettled.json`, 의뢰서 0절). 넘기는 사유는
셋뿐이다:

| 사유 | 뜻 |
|---|---|
| `tie` | 두 심볼이 같은 값이라 전역 우선순위(=관례)로 이겼다 |
| `heur` | 등록된 규칙이 없어 산술로 지어낸 이름이다 |
| `bare` | 이름을 못 붙였는데 크기가 커서(≥64) 진짜 차원일 수 있다 |

크기-1 축과 루프 카운터는 **이름이 없는 게 정답**이라 넘기지 않는다(이름 없는 축의 97.5%).

같이 넘기는 것: 실측 크기, 후보 심볼, 그 축이 지나는 모듈, 그리고 **붙여넣으면 되는 override
초안**. 답이 나오면 `spread: class` 로 그 축이 지나는 모든 자리가 한 번에 바뀐다.
현재 44개 모델에 **질문 268개**이고, **15개 모델은 질문이 0개**다.
...
| 하드 불변식 위반 | **0** — 위 §3② 표의 19종 전부, prefill·decode 양쪽 |
| 게이트 | FAIL 0 · 셀프테스트 32/32 (죽은 검사 0) |
