...
# 배치 E — 대상 모델: deepseek-ai__DeepSeek-V3, ibm-granite__granite-4.0-h-small, meta-llama__Llama-3.1-405B, tencent__Hunyuan-A13B-Instruct


# ============================================================
# 모델: deepseek-ai__DeepSeek-V3
# ============================================================

# 리뷰 패킷 — deepseek-ai/DeepSeek-V3

> 이 문서는 **자기완결적**입니다. 판단에 필요한 것은 전부 아래에 있습니다.
> revision `e815299b0bcbac849fa540c768ef21845365c9eb` / 트레이스 seq_len(T) = 24
...
shape 축 **220,354개**를 렌더하면서 어떤 근거로 이름을 붙였는지의 내역이다. 위쪽 네 줄은 `rules/`에 **등록된 규칙**이 답을 준 경우이고, `휴리스틱`으로 시작하는 줄은 등록된 규칙이 없어 **산술적으로 맞는 이름을 지어낸** 경우다. 후자는 이번 트레이스의 seq_len에서만 참일 수 있으므로 그대로 신뢰하면 안 되고, `02-new-module-handling.md` Tier 2로 확인해 규칙으로 승격시켜야 한다.

| 근거 | 축 수 | 비율 |
|---|---:|---:|
| 런타임 축 (B/T/1) | 72,809 | 33.04% |
| 스코프 없는 심볼 | 61,185 | 27.77% |
| 이 모듈 스코프의 심볼 | 53,317 | 24.20% |
| 이 모듈 스코프의 유도식 | 29,949 | 13.59% |
| 같은 shape에서 이미 쓴 심볼 재사용 | 2,328 | 1.06% |
| 이름 없음 (정수 유지) | 766 | 0.35% |

등록된 규칙 **217,260축**, 약한 근거 2,328축, 휴리스틱 **0축 (0.0%)**, 이름 없음 766축.
...
| 24576 | n_h·(d_nope+d_rope) (MLA q_b_proj 출력) | q_b_proj, self_attn |
...
| C6 | PASS | hidden_size=7168 (heuristic check, 8350 flagged) |
...
| C16 | INFO | 6620 unmapped rows, 32 distinct raw ops: ['aten._to_copy.default', 'aten._unsafe_view.default', '... |
...
C16  INFO   6620 unmapped rows, 32 distinct raw ops: ['aten._to_copy.default', 'aten._unsafe_view.default', 'aten.alias.default', 'aten.arange.default', 'aten.bitwise_not.default', 'aten.clamp_.default', 'aten.clone.default', 'aten.div_.Tensor', 'aten.empty_like.default', 'aten.expand.default']
C17  PASS   유도 상수 전부 설명됨, 구조 라이브러리에 등재됨
...
```
  ...
  model.layers.N.mlp.gate                            view             [T,E] -> [T,n_grp,E/n_grp]
  model.layers.N.mlp.gate                            topk             [T,n_grp,E/n_grp] -> [T,n_grp,2]*[T,n_grp,2]
  model.layers.N.mlp.gate                            sum              [T,n_grp,2] -> [T,n_grp]
  model.layers.N.mlp.gate                            topk             [T,n_grp] -> [T,k_grp]*[T,k_grp]
  model.layers.N.mlp.gate                            zeros_like       [T,n_grp] -> [T,n_grp]
  model.layers.N.mlp.gate                            scatter_         [T,n_grp]*[T,k_grp] -> [T,n_grp]
  model.layers.N.mlp.gate                            unsqueeze        [T,n_grp] -> [T,n_grp,1]
  model.layers.N.mlp.gate                            expand           [T,n_grp,1] -> [T,n_grp,E/n_grp]
  model.layers.N.mlp.gate                            clone            [T,n_grp,E/n_grp] -> [T,n_grp,E/n_grp]
  model.layers.N.mlp.gate                            _unsafe_view     [T,n_grp,E/n_grp] -> [T,E]
...
```
  ...
  model.layers.N.mlp.gate                            view             [B,E] -> [B,n_grp,E/n_grp]
  model.layers.N.mlp.gate                            topk             [B,n_grp,E/n_grp] -> [B,n_grp,2]*[B,n_grp,2]
  model.layers.N.mlp.gate                            sum              [B,n_grp,2] -> [B,n_grp]
  model.layers.N.mlp.gate                            topk             [B,n_grp] -> [B,k_grp]*[B,k_grp]
  model.layers.N.mlp.gate                            zeros_like       [B,n_grp] -> [B,n_grp]
  model.layers.N.mlp.gate                            scatter_         [B,n_grp]*[B,k_grp] -> [B,n_grp]
  model.layers.N.mlp.gate                            unsqueeze        [B,n_grp] -> [B,n_grp,1]
  model.layers.N.mlp.gate                            expand           [B,n_grp,1] -> [B,n_grp,E/n_grp]
  model.layers.N.mlp.gate                            clone            [B,n_grp,E/n_grp] -> [B,n_grp,E/n_grp]
  model.layers.N.mlp.gate                            _unsafe_view     [B,n_grp,E/n_grp] -> [B,E]
...
# ============================================================
# 모델: ibm-granite__granite-4.0-h-small
# ============================================================
...
| 8 | KEY DETAIL | GQA attention; Sparse MoE (E=72, top-10, topk-then-softmax routing) |
| 9 | Related concepts | RMSNorm, RoPE, GQA, MoE, topk-softmax routing, short-conv (SSM/DeltaNet) |
...
```
  ...
  model.layers.N.mamba                               elementwise_add  [B,T,n_h_ssm]*[d_state] -> [B,T,n_h_ssm]
  ...
  model.layers.N.mamba                               unsqueeze        [d_state] -> [d_state,1]
  model.layers.N.mamba                               constant_pad_nd  [B,T,n_h_ssm,d_head_ssm] -> [B,d_chunk,n_h_ssm,d_head_ssm]
  model.layers.N.mamba                               elementwise_mul  [d_state,1]*[B,d_chunk,n_h_ssm,d_head_ssm] -> [B,d_chunk,n_h_ssm,d_head_ssm]
  ...
  model.layers.N.mamba                               constant_pad_nd  [B,T,n_h_ssm,d_head_ssm] -> [B,d_chunk,d_state,d_head_ssm]
  ...
  model.layers.N.mamba                               constant_pad_nd  [B,T,d_state,n_h_ssm] -> [B,d_chunk,n_h_ssm,d_state]
  ...
  model.layers.N.mamba                               elementwise_mul  [B,1,d_chunk,1,d_state,n_h_ssm]*[B,1,1,d_chunk,n_h_ssm,d_state] -> [B,1,d_chunk,d_chunk,d_state,n_h_ssm]
  model.layers.N.mamba                               sum              [B,1,d_chunk,d_chunk,d_state,n_h_ssm] -> [B,1,d_chunk,d_chunk,n_h_ssm]
  ...
  model.layers.N.mamba                               elementwise_mul  [B,1,d_chunk,d_chunk,n_h_ssm,1]*[B,1,1,d_chunk,d_state,d_head_ssm] -> [B,1,d_chunk,d_chunk,n_h_ssm,d_head_ssm]
  model.layers.N.mamba                               sum              [B,1,d_chunk,d_chunk,n_h_ssm,d_head_ssm] -> [B,1,d_chunk,n_h_ssm,d_head_ssm]
  model.layers.N.mamba                               slice            [B,n_h_ssm,1,d_chunk] -> [B,d_state,1,1]
  model.layers.N.mamba                               sub              [B,d_state,1,1]*[B,n_h_ssm,1,d_chunk] -> [B,n_h_ssm,1,d_chunk]
  ...
  model.layers.N.mamba                               expand           [B,d_state,2,1] -> [B,n_h_ssm,2,2]
  ...
  model.layers.N.block_sparse_moe.experts            unsqueeze        [k*T] -> [k*T,1]
  model.layers.N.block_sparse_moe.experts            clamp_           [k*T] -> [k*T]
  model.layers.N.block_sparse_moe.experts            masked_fill_     [k*T,d_model]*[k*T,1] -> [k*T,d_model]
  ...
  model.layers.N.block_sparse_moe.experts            elementwise_mul  [k*T,d_model]*[k*T,1] -> [k*T,d_model]
...
```
  ...
  model.layers.N.mamba                               expand           [B,d_state,1] -> [B,n_h_ssm,d_head_ssm]
  model.layers.N.mamba                               unsqueeze        [d_state] -> [d_state,1]
  model.layers.N.mamba                               expand           [d_state,1] -> [d_state,d_head_ssm]
  model.layers.N.mamba                               elementwise_add  [B,n_h_ssm,d_head_ssm]*[d_state,d_head_ssm] -> [B,n_h_ssm,d_head_ssm]
  ...
  model.layers.N.mamba                               expand           [n_h_ssm,1,1] -> [d_state,d_head_ssm,n_h_ssm]
  model.layers.N.mamba                               unsqueeze        [B,n_h_ssm,d_head_ssm] -> [B,n_h_ssm,d_head_ssm,1]
  model.layers.N.mamba                               elementwise_mul  [B,n_h_ssm,d_head_ssm,1]*[d_state,d_head_ssm,n_h_ssm] -> [B,n_h_ssm,d_head_ssm,d_state]
  model.layers.N.mamba                               exp              [B,n_h_ssm,d_head_ssm,d_state] -> [B,n_h_ssm,d_head_ssm,d_state]
  model.layers.N.mamba                               view             [B,n_h_ssm] -> [B,1,n_h_ssm]
  model.layers.N.mamba                               unsqueeze        [B,1,n_h_ssm] -> [B,1,1,n_h_ssm]
  model.layers.N.mamba                               expand           [B,1,1,n_h_ssm] -> [B,1,d_state,d_state]
  model.layers.N.mamba                               clone            [B,1,d_state,d_state] -> [B,1,d_state,d_state]
  ...
  model.layers.N.block_sparse_moe.experts            unsqueeze        [k] -> [k,1]
  model.layers.N.block_sparse_moe.experts            clamp_           [k] -> [k]
  model.layers.N.block_sparse_moe.experts            masked_fill_     [k,d_model]*[k,1] -> [k,d_model]
  model.layers.N.block_sparse_moe.experts            transpose        [E,2*d_moe,d_model] -> w=[E,2*d_moe,d_model] [E,d_model,2*d_moe]
  ...
  model.layers.N.block_sparse_moe.experts            elementwise_mul  [k,d_model]*[k,1] -> [k,d_model]
...
# ============================================================
# 모델: meta-llama__Llama-3.1-405B
# ============================================================

# 리뷰 패킷 — meta-llama/Llama-3.1-405B

> 이 문서는 **자기완결적**입니다. 판단에 필요한 것은 전부 아래에 있습니다.
> revision `b906e4dc842aa489c962f9db26554dcfdde901fe` / 트레이스 seq_len(T) = 17
> 라이브러리: torch 2.13.0+cpu, transformers 5.14.1
...
# Model Summary -- meta-llama/Llama-3.1-405B

## 기본 정보

- revision: `b906e4dc842aa489c962f9db26554dcfdde901fe`
- capture backend: meta (meta/fake device, 실제 가중치 연산 없음)
- 트레이스 seq_len (T): 17
- attn_implementation: None
- 라이브러리: torch 2.13.0+cpu, transformers 5.14.1
...
| 5 | Attention | GQA |
| 6 | LAYER MIX | 126× GQA |
| 7 | KV CACHE / TOKEN (BF16) | 504.0 KiB (Very high) |
...
|---|---|
| 모델 타입 (config) | `llama` |
| attention | GQA — 128 query : 8 kv heads (repeat 16), d_head=128 |
| attention 커널 | eager (explicit softmax) |
| 위치 인코딩 | RoPE (θ=500000.0), llama3 scaling |
| FFN | dense FFN — intermediate 53248, SwiGLU (silu·gate) |
| 정규화 | RMSNorm |
| tie embeddings | False |
| decode 방식 | autoregressive, 1 token/step, reuses KV cache (prefill builds it) |
...
shape 축 **284,314개**를 렌더하면서 어떤 근거로 이름을 붙였는지의 내역이다. 위쪽 네 줄은 `rules/`에 **등록된 규칙**이 답을 준 경우이고, `휴리스틱`으로 시작하는 줄은 등록된 규칙이 없어 **산술적으로 맞는 이름을 지어낸** 경우다. 후자는 이번 트레이스의 seq_len에서만 참일 수 있으므로 그대로 신뢰하면 안 되고, `02-new-module-handling.md` Tier 2로 확인해 규칙으로 승격시켜야 한다.

| 근거 | 축 수 | 비율 |
|---|---:|---:|
| 런타임 축 (B/T/1) | 92,879 | 32.67% |
| 이 모듈 스코프의 심볼 | 79,813 | 28.07% |
| 스코프 없는 심볼 | 70,829 | 24.91% |
| 이 모듈 스코프의 유도식 | 35,491 | 12.48% |
| 같은 shape에서 이미 쓴 심볼 재사용 | 4,798 | 1.69% |
| 이름 없음 (정수 유지) | 504 | 0.18% |

등록된 규칙 **279,012축**, 약한 근거 4,798축, 휴리스틱 **0축 (0.0%)**, 이름 없음 504축.
...
| 값 | 유래 | 나타나는 모듈 |
|---|---|---|
| 16 | n_h/n_kv (GQA repeat 계수 — repeat_kv의 expand 축) | self_attn |
| 64 | d_head/2 (RoPE rotate_half 분할 축) | rotary_emb, self_attn |
| 1024 | n_kv·d_head (KV 투영 폭) | k_proj, self_attn, v_proj |

## 레이어 구조

- layer 0-125: input_layernorm, mlp, post_attention_layernorm, self_attn
...
| C6 | PASS | hidden_size=16384 (heuristic check, 8442 flagged) |
...
| C15 | PASS | all discovered entrypoints traced |
| C16 | INFO | 7725 unmapped rows, 14 distinct raw ops: ['aten._to_copy.default', 'aten._unsafe_view.default', '... |
| C17 | PASS | 유도 상수 전부 설명됨, 구조 라이브러리에 등재됨 |
...
| 구분 | 소스 | 역할 |
|---|---|---|
| config (1차) | HF `meta-llama/Llama-3.1-405B` config.json @ `b906e4dc842aa489c962f9db26554dcfdde901fe` (sha256 `a64ce9627064…`) | 심볼 값의 출처 |
| modeling code (1차) | transformers 5.14.1 공식 modeling forward (meta device) | op·shape·dependency 캡처 |
...
C14  PASS   used=17 >= required=16
C15  PASS   all discovered entrypoints traced
C16  INFO   7725 unmapped rows, 14 distinct raw ops: ['aten._to_copy.default', 'aten._unsafe_view.default', 'aten.alias.default', 'aten.arange.default', 'aten.clone.default', 'aten.expand.default', 'aten.le.Tensor', 'aten.scalar_tensor.default', 'aten.slice.Tensor', 'aten.t.default']
C17  PASS   유도 상수 전부 설명됨, 구조 라이브러리에 등재됨
...
```
  ...
  model.layers.108                                   elementwise_add  [B,T,d_model]*[B,T,d_model] -> [B,T,d_model]
  model.layers.109                                   elementwise_add  [B,T,d_model]*[B,T,d_model] -> [B,T,d_model]
  model.layers.110                                   elementwise_add  [B,T,d_model]*[B,T,d_model] -> [B,T,d_model]
  model.layers.111                                   elementwise_add  [B,T,d_model]*[B,T,d_model] -> [B,T,d_model]
  model.layers.112                                   elementwise_add  [B,T,d_model]*[B,T,d_model] -> [B,T,d_model]
  model.layers.113                                   elementwise_add  [B,T,d_model]*[B,T,d_model] -> [B,T,d_model]
  model.layers.114                                   elementwise_add  [B,T,d_model]*[B,T,d_model] -> [B,T,d_model]
  model.layers.115                                   elementwise_add  [B,T,d_model]*[B,T,d_model] -> [B,T,d_model]
  model.layers.116                                   elementwise_add  [B,T,d_model]*[B,T,d_model] -> [B,T,d_model]
  model.layers.117                                   elementwise_add  [B,T,d_model]*[B,T,d_model] -> [B,T,d_model]
  model.layers.118                                   elementwise_add  [B,T,d_model]*[B,T,d_model] -> [B,T,d_model]
  model.layers.119                                   elementwise_add  [B,T,d_model]*[B,T,d_model] -> [B,T,d_model]
  model.layers.120                                   elementwise_add  [B,T,d_model]*[B,T,d_model] -> [B,T,d_model]
  model.layers.121                                   elementwise_add  [B,T,d_model]*[B,T,d_model] -> [B,T,d_model]
  model.layers.122                                   elementwise_add  [B,T,d_model]*[B,T,d_model] -> [B,T,d_model]
  model.layers.123                                   elementwise_add  [B,T,d_model]*[B,T,d_model] -> [B,T,d_model]
  model.layers.124                                   elementwise_add  [B,T,d_model]*[B,T,d_model] -> [B,T,d_model]
  model.layers.125                                   elementwise_add  [B,T,d_model]*[B,T,d_model] -> [B,T,d_model]
  model.norm                                         _to_copy         [B,T,d_model] -> [B,T,d_model]
  model.norm                                         pow              [B,T,d_model] -> [B,T,d_model]
  model.norm                                         mean             [B,T,d_model] -> [B,T,1]
  model.norm                                         elementwise_add  [B,T,1] -> [B,T,1]
...
```
  ...
  model.layers.108                                   elementwise_add  [B,1,d_model]*[B,1,d_model] -> [B,1,d_model]
  model.layers.109                                   elementwise_add  [B,1,d_model]*[B,1,d_model] -> [B,1,d_model]
  model.layers.110                                   elementwise_add  [B,1,d_model]*[B,1,d_model] -> [B,1,d_model]
  model.layers.111                                   elementwise_add  [B,1,d_model]*[B,1,d_model] -> [B,1,d_model]
  model.layers.112                                   elementwise_add  [B,1,d_model]*[B,1,d_model] -> [B,1,d_model]
  model.layers.113                                   elementwise_add  [B,1,d_model]*[B,1,d_model] -> [B,1,d_model]
  model.layers.114                                   elementwise_add  [B,1,d_model]*[B,1,d_model] -> [B,1,d_model]
  model.layers.115                                   elementwise_add  [B,1,d_model]*[B,1,d_model] -> [B,1,d_model]
  model.layers.116                                   elementwise_add  [B,1,d_model]*[B,1,d_model] -> [B,1,d_model]
  model.layers.117                                   elementwise_add  [B,1,d_model]*[B,1,d_model] -> [B,1,d_model]
  model.layers.118                                   elementwise_add  [B,1,d_model]*[B,1,d_model] -> [B,1,d_model]
  model.layers.119                                   elementwise_add  [B,1,d_model]*[B,1,d_model] -> [B,1,d_model]
  model.layers.120                                   elementwise_add  [B,1,d_model]*[B,1,d_model] -> [B,1,d_model]
  model.layers.121                                   elementwise_add  [B,1,d_model]*[B,1,d_model] -> [B,1,d_model]
  model.layers.122                                   elementwise_add  [B,1,d_model]*[B,1,d_model] -> [B,1,d_model]
  model.layers.123                                   elementwise_add  [B,1,d_model]*[B,1,d_model] -> [B,1,d_model]
  model.layers.124                                   elementwise_add  [B,1,d_model]*[B,1,d_model] -> [B,1,d_model]
  model.layers.125                                   elementwise_add  [B,1,d_model]*[B,1,d_model] -> [B,1,d_model]
  model.norm                                         _to_copy         [B,1,d_model] -> [B,1,d_model]
  model.norm                                         pow              [B,1,d_model] -> [B,1,d_model]
  model.norm                                         mean             [B,1,d_model] -> [B,1,1]
  model.norm                                         elementwise_add  [B,1,1] -> [B,1,1]
...
# ============================================================
# 모델: tencent__Hunyuan-A13B-Instruct
# ============================================================
...
| tie embeddings | True |
| decode 방식 | autoregressive, 1 token/step, reuses KV cache (prefill builds it) |
| KV cache 크기 | 2·n_kv·d_head = 2·8·128 = 2048 elems / token / layer; 32 attention layer(s) ⇒ 65536 / token |
