...
# 배치 N — 대상 모델: mistralai__Mistral-Small-3.2-24B-Instruct-2506, Qwen__Qwen3-30B-A3B, baidu__ERNIE-4.5-21B-A3B-PT, LiquidAI__LFM2-8B-A1B
...
| 8 | KEY DETAIL | GQA attention; Sparse MoE (E=128, top-8, topk-then-softmax routing) |
| 9 | Related concepts | RMSNorm, RoPE, GQA, MoE, topk-softmax routing, QK-Norm |
...
shape 축 **143,940개**를 렌더하면서 어떤 근거로 이름을 붙였는지의 내역이다. 위쪽 네 줄은 `rules/`에 **등록된 규칙**이 답을 준 경우이고, `휴리스틱`으로 시작하는 줄은 등록된 규칙이 없어 **산술적으로 맞는 이름을 지어낸** 경우다. 후자는 이번 트레이스의 seq_len에서만 참일 수 있으므로 그대로 신뢰하면 안 되고, `02-new-module-handling.md` Tier 2로 확인해 규칙으로 승격시켜야 한다.

| 근거 | 축 수 | 비율 |
|---|---:|---:|
| 이 모듈 스코프의 심볼 | 44,845 | 31.16% |
| 런타임 축 (B/T/1) | 43,913 | 30.51% |
| 스코프 없는 심볼 | 37,141 | 25.80% |
| 이 모듈 스코프의 유도식 | 16,015 | 11.13% |
| 같은 shape에서 이미 쓴 심볼 재사용 | 1,834 | 1.27% |
| 이름 없음 (정수 유지) | 192 | 0.13% |

등록된 규칙 **141,914축**, 약한 근거 1,834축, 휴리스틱 **0축 (0.0%)**, 이름 없음 192축.
...
| C6 | PASS | hidden_size=2048 (heuristic check, 5136 flagged) |
...
| C15 | PASS | all discovered entrypoints traced |
| C16 | INFO | 4167 unmapped rows, 25 distinct raw ops: ['aten._to_copy.default', 'aten._unsafe_view.default', '... |
| C17 | PASS | 유도 상수 전부 설명됨, 구조 라이브러리에 등재됨 |
...
C14  PASS   used=104 >= required=104
C15  PASS   all discovered entrypoints traced
C16  INFO   4167 unmapped rows, 25 distinct raw ops: ['aten._to_copy.default', 'aten._unsafe_view.default', 'aten.alias.default', 'aten.arange.default', 'aten.clamp_.default', 'aten.clone.default', 'aten.div_.Tensor', 'aten.empty_like.default', 'aten.expand.default', 'aten.floor_divide.default']
C17  PASS   유도 상수 전부 설명됨, 구조 라이브러리에 등재됨
...
| 8 | KEY DETAIL | GQA attention; Sparse MoE (E=64, top-6, +2 shared, topk-then-softmax routing); dense-prefix 1 layer(s) |
| 9 | Related concepts | RMSNorm, RoPE, GQA, MoE, shared expert, topk-softmax routing, MTP |
...
| 9 | Related concepts | RMSNorm, RoPE, GQA, MoE, sigmoid-gating, short-conv |
...
```
  ...
  model.layers.N.feed_forward.experts                unsqueeze        [k*T] -> [k*T,1]
  model.layers.N.feed_forward.experts                clamp_           [k*T] -> [k*T]
  model.layers.N.feed_forward.experts                masked_fill_     [k*T,d_model]*[k*T,1] -> [k*T,d_model]
  ...
  model.layers.N.feed_forward.experts                elementwise_mul  [k*T,d_model]*[k*T,1] -> [k*T,d_model]
...
```
  ...
  model.layers.N.feed_forward.experts                unsqueeze        [k] -> [k,1]
  model.layers.N.feed_forward.experts                clamp_           [k] -> [k]
  model.layers.N.feed_forward.experts                masked_fill_     [k,d_model]*[k,1] -> [k,d_model]
  model.layers.N.feed_forward.experts                transpose        [E,2*d_moe,d_model] -> w=[E,2*d_moe,d_model] [E,d_model,2*d_moe]
  ...
  model.layers.N.feed_forward.experts                elementwise_mul  [k,d_model]*[k,1] -> [k,d_model]
