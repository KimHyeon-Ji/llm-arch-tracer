...
# 배치 P — 대상 모델: zai-org__GLM-4.5-Air, moonshotai__Kimi-Linear-48B-A3B-Instruct


# ============================================================
# 모델: zai-org__GLM-4.5-Air
# ============================================================
...
shape 축 **150,769개**를 렌더하면서 어떤 근거로 이름을 붙였는지의 내역이다. 위쪽 네 줄은 `rules/`에 **등록된 규칙**이 답을 준 경우이고, `휴리스틱`으로 시작하는 줄은 등록된 규칙이 없어 **산술적으로 맞는 이름을 지어낸** 경우다. 후자는 이번 트레이스의 seq_len에서만 참일 수 있으므로 그대로 신뢰하면 안 되고, `02-new-module-handling.md` Tier 2로 확인해 규칙으로 승격시켜야 한다.

| 근거 | 축 수 | 비율 |
|---|---:|---:|
| 이 모듈 스코프의 심볼 | 49,656 | 32.94% |
| 런타임 축 (B/T/1) | 44,735 | 29.67% |
| 스코프 없는 심볼 | 43,347 | 28.75% |
| 이 모듈 스코프의 유도식 | 10,684 | 7.09% |
| 같은 shape에서 이미 쓴 심볼 재사용 | 1,758 | 1.17% |
| 이름 없음 (정수 유지) | 589 | 0.39% |

등록된 규칙 **148,422축**, 약한 근거 1,758축, 휴리스틱 **0축 (0.0%)**, 이름 없음 589축.
...
| C6 | PASS | hidden_size=4096 (heuristic check, 5478 flagged) |
...
| C16 | INFO | 4919 unmapped rows, 30 distinct raw ops: ['aten._to_copy.default', 'aten._unsafe_view.default', '... |
...
C16  INFO   4919 unmapped rows, 30 distinct raw ops: ['aten._to_copy.default', 'aten._unsafe_view.default', 'aten.alias.default', 'aten.arange.default', 'aten.bitwise_not.default', 'aten.clamp_.default', 'aten.clone.default', 'aten.div_.Tensor', 'aten.empty_like.default', 'aten.expand.default']
C17  PASS   유도 상수 전부 설명됨, 구조 라이브러리에 등재됨
...
# ============================================================
# 모델: moonshotai__Kimi-Linear-48B-A3B-Instruct
# ============================================================

# 리뷰 패킷 — moonshotai/Kimi-Linear-48B-A3B-Instruct

> 이 문서는 **자기완결적**입니다. 판단에 필요한 것은 전부 아래에 있습니다.
> revision `e1df551a447157d4658b573f9a695d57658590e9` / 트레이스 seq_len(T) = 16
> 라이브러리: torch 2.13.0+cpu, transformers 5.14.1
...
```
  ...
  model.layers.N.self_attn.q_proj                    view             [B,T,d_model] -> [T,d_model]
  model.layers.N.self_attn.q_proj                    matmul           [T,d_model]*[d_model,n_h_kda*d_head_kda] -> w=[n_h_kda*d_head_kda,d_model] [T,n_h_kda*d_head_kda]
  model.layers.N.self_attn.q_proj                    _unsafe_view     [T,n_h_kda*d_head_kda] -> [B,T,n_h_kda*d_head_kda]
  ...
  model.layers.N.self_attn.k_proj                    view             [B,T,d_model] -> [T,d_model]
  model.layers.N.self_attn.k_proj                    matmul           [T,d_model]*[d_model,n_h_kda*d_head_kda] -> w=[n_h_kda*d_head_kda,d_model] [T,n_h_kda*d_head_kda]
  model.layers.N.self_attn.k_proj                    _unsafe_view     [T,n_h_kda*d_head_kda] -> [B,T,n_h_kda*d_head_kda]
  ...
  model.layers.N.self_attn.v_proj                    view             [B,T,d_model] -> [T,d_model]
  model.layers.N.self_attn.v_proj                    matmul           [T,d_model]*[d_model,n_h_kda*d_head_kda] -> w=[n_h_kda*d_head_kda,d_model] [T,n_h_kda*d_head_kda]
  model.layers.N.self_attn.v_proj                    _unsafe_view     [T,n_h_kda*d_head_kda] -> [B,T,n_h_kda*d_head_kda]
  ...
  model.layers.N.self_attn.f_b_proj                  matmul           [T,d_head_kda]*[d_head_kda,n_h_kda*d_head_kda] -> w=[n_h_kda*d_head_kda,d_head_kda] [T,n_h_kda*d_head_kda]
  model.layers.N.self_attn.f_b_proj                  _unsafe_view     [T,n_h_kda*d_head_kda] -> [B,T,n_h_kda*d_head_kda]
  ...
  model.layers.N.self_attn                           view             [1,1,n_h_kda,1] -> w=[1,1,n_h_kda,1] [n_h_kda,1]
  model.layers.N.self_attn                           exp              [n_h_kda,1] -> w=[1,1,n_h_kda,1] [n_h_kda,1]
  ...
  model.layers.N.self_attn                           unsqueeze        [B,T,n_h_kda,d_head_kda] -> [B,T,n_h_kda,1,d_head_kda]
  model.layers.N.self_attn                           expand           [B,T,n_h_kda,1,d_head_kda] -> [B,T,n_h_kda,1,d_head_kda]
  model.layers.N.self_attn                           clone            [B,T,n_h_kda,1,d_head_kda] -> [B,T,n_h_kda,1,d_head_kda]
  model.layers.N.self_attn                           view             [B,T,n_h_kda,1,d_head_kda] -> [B,T,n_h_kda,d_head_kda]
  model.layers.N.self_attn                           elementwise_mul  [B,T,n_h_kda,d_head_kda] -> [B,T,n_h_kda,d_head_kda]
  model.layers.N.self_attn                           new_zeros        [B,T,n_h_kda,d_head_kda] -> [B,n_h_kda,d_head_kda,d_head_kda]
  model.layers.N.self_attn                           zeros_like       [B,T,n_h_kda,d_head_kda] -> [B,T,n_h_kda,d_head_kda]
  model.layers.N.self_attn                           select           [B,T,n_h_kda,d_head_kda] -> [B,n_h_kda,d_head_kda]
  model.layers.N.self_attn                           select           [B,T,n_h_kda] -> [B,n_h_kda]
  ...
  model.layers.N.self_attn.g_a_proj                  t                [d_head_kda,d_model] -> w=[d_head_kda,d_model] [d_model,d_head_kda]
  model.layers.N.self_attn.g_a_proj                  view             [B,T,d_model] -> [T,d_model]
  model.layers.N.self_attn.g_a_proj                  matmul           [T,d_model]*[d_model,d_head_kda] -> w=[d_head_kda,d_model] [T,d_head_kda]
  model.layers.N.self_attn.g_a_proj                  _unsafe_view     [T,d_head_kda] -> [B,T,d_head_kda]
  model.layers.N.self_attn.g_b_proj                  t                [n_h_kda*d_head_kda,d_head_kda] -> w=[n_h_kda*d_head_kda,d_head_kda] [d_head_kda,n_h_kda*d_head_kda]
  model.layers.N.self_attn.g_b_proj                  view             [B,T,d_head_kda] -> [T,d_head_kda]
  model.layers.N.self_attn.g_b_proj                  matmul           [T,d_head_kda]*[d_head_kda,n_h_kda*d_head_kda] -> w=[n_h_kda*d_head_kda,d_head_kda] [T,n_h_kda*d_head_kda]
  model.layers.N.self_attn.g_b_proj                  _unsafe_view     [T,n_h_kda*d_head_kda] -> [B,T,n_h_kda*d_head_kda]
  ...
  model.layers.N.block_sparse_moe.gate               add_             [T,E]*[B,E] -> [T,E]
  model.layers.N.block_sparse_moe.gate               view             [T,E] -> [T,1,E]
  model.layers.N.block_sparse_moe.gate               topk             [T,1,E] -> [T,1,2]*[T,1,2]
  model.layers.N.block_sparse_moe.gate               sum              [T,1,2] -> [T,1]
  model.layers.N.block_sparse_moe.gate               topk             [T,1] -> [T,1]*[T,1]
  model.layers.N.block_sparse_moe.gate               zeros_like       [T,1] -> [T,1]
  model.layers.N.block_sparse_moe.gate               scatter_         [T,1]*[T,1] -> [T,1]
  model.layers.N.block_sparse_moe.gate               unsqueeze        [T,1] -> [T,1,1]
  model.layers.N.block_sparse_moe.gate               expand           [T,1,1] -> [T,1,E]
  model.layers.N.block_sparse_moe.gate               view             [T,1,E] -> [T,E]
  model.layers.N.block_sparse_moe.gate               _to_copy         [T,E] -> [T,E]
  model.layers.N.block_sparse_moe.gate               bitwise_not      [T,E] -> [T,E]
  model.layers.N.block_sparse_moe.gate               masked_fill      [T,E]*[T,E] -> [T,E]
  ...
  model.layers.N.block_sparse_moe                    index            [T,d_model]*[k*T] -> [k*T,d_model]
  model.layers.N.block_sparse_moe                    slice            [k*T,d_model] -> [4*k,d_model]
  model.layers.N.block_sparse_moe.experts.N.w1       t                [d_moe,d_model] -> w=[d_moe,d_model] [d_model,d_moe]
  model.layers.N.block_sparse_moe.experts.N.w1       matmul           [4*k,d_model]*[d_model,d_moe] -> w=[d_moe,d_model] [4*k,d_moe]
  model.layers.N.block_sparse_moe.experts.N.act_fn   silu             [4*k,d_moe] -> [4*k,d_moe]
  model.layers.N.block_sparse_moe.experts.N.w3       t                [d_moe,d_model] -> w=[d_moe,d_model] [d_model,d_moe]
  model.layers.N.block_sparse_moe.experts.N.w3       matmul           [4*k,d_model]*[d_model,d_moe] -> w=[d_moe,d_model] [4*k,d_moe]
  model.layers.N.block_sparse_moe.experts.0          elementwise_mul  [4*k,d_moe]*[4*k,d_moe] -> [4*k,d_moe]
  model.layers.N.block_sparse_moe.experts.N.w2       t                [d_model,d_moe] -> w=[d_model,d_moe] [d_moe,d_model]
  model.layers.N.block_sparse_moe.experts.N.w2       matmul           [4*k,d_moe]*[d_moe,d_model] -> w=[d_model,d_moe] [4*k,d_model]
  model.layers.N.block_sparse_moe.experts.1          elementwise_mul  [4*k,d_moe]*[4*k,d_moe] -> [4*k,d_moe]
  model.layers.N.block_sparse_moe.experts.2          elementwise_mul  [4*k,d_moe]*[4*k,d_moe] -> [4*k,d_moe]
  model.layers.N.block_sparse_moe.experts.3          elementwise_mul  [4*k,d_moe]*[4*k,d_moe] -> [4*k,d_moe]
  model.layers.N.block_sparse_moe                    concat           [4*k,d_model]*[4*k,d_model]*[4*k,d_model]*[4*k,d_model] -> [k*T,d_model]
  model.layers.N.block_sparse_moe                    empty_like       [k*T,d_model] -> [k*T,d_model]
  model.layers.N.block_sparse_moe                    index_put_       [k*T,d_model]*[k*T]*[k*T,d_model] -> [k*T,d_model]
  model.layers.N.block_sparse_moe                    view             [k*T,d_model] -> [T,k,d_model]
  model.layers.N.block_sparse_moe                    _to_copy         [T,k,d_model] -> [T,k,d_model]
  model.layers.N.block_sparse_moe                    unsqueeze        [T,k] -> [T,k,1]
  model.layers.N.block_sparse_moe                    mul_             [T,k,d_model]*[T,k,1] -> [T,k,d_model]
  model.layers.N.block_sparse_moe                    sum              [T,k,d_model] -> [T,d_model]
  model.layers.N.block_sparse_moe                    _to_copy         [T,d_model] -> [T,d_model]
  model.layers.N.block_sparse_moe                    view             [T,d_model] -> [B,T,d_model]
  model.layers.N.block_sparse_moe.shared_experts.gate_proj t                [d_moe,d_model] -> w=[d_moe,d_model] [d_model,d_moe]
  model.layers.N.block_sparse_moe.shared_experts.gate_proj view             [B,T,d_model] -> [T,d_model]
  model.layers.N.block_sparse_moe.shared_experts.gate_proj matmul           [T,d_model]*[d_model,d_moe] -> w=[d_moe,d_model] [T,d_moe]
  model.layers.N.block_sparse_moe.shared_experts.gate_proj _unsafe_view     [T,d_moe] -> [B,T,d_moe]
  model.layers.N.block_sparse_moe.shared_experts.act_fn silu             [B,T,d_moe] -> [B,T,d_moe]
  model.layers.N.block_sparse_moe.shared_experts.up_proj t                [d_moe,d_model] -> w=[d_moe,d_model] [d_model,d_moe]
  model.layers.N.block_sparse_moe.shared_experts.up_proj view             [B,T,d_model] -> [T,d_model]
  model.layers.N.block_sparse_moe.shared_experts.up_proj matmul           [T,d_model]*[d_model,d_moe] -> w=[d_moe,d_model] [T,d_moe]
  model.layers.N.block_sparse_moe.shared_experts.up_proj _unsafe_view     [T,d_moe] -> [B,T,d_moe]
  model.layers.N.block_sparse_moe.shared_experts     elementwise_mul  [B,T,d_moe]*[B,T,d_moe] -> [B,T,d_moe]
  model.layers.N.block_sparse_moe.shared_experts.down_proj t                [d_model,d_moe] -> w=[d_model,d_moe] [d_moe,d_model]
  model.layers.N.block_sparse_moe.shared_experts.down_proj view             [B,T,d_moe] -> [T,d_moe]
  model.layers.N.block_sparse_moe.shared_experts.down_proj matmul           [T,d_moe]*[d_moe,d_model] -> w=[d_model,d_moe] [T,d_model]
...
```
  ...
  model.layers.N.self_attn                           copy_            [B,n_h_kda,d_head_kda]*[B,n_h_kda,d_head_kda] -> [B,n_h_kda,d_head_kda]
  model.layers.N.self_attn.g_a_proj                  t                [d_head_kda,d_model] -> w=[d_head_kda,d_model] [d_model,d_head_kda]
  model.layers.N.self_attn.g_a_proj                  view             [B,1,d_model] -> [B,d_model]
  model.layers.N.self_attn.g_a_proj                  matmul           [B,d_model]*[d_model,d_head_kda] -> w=[d_head_kda,d_model] [B,d_head_kda]
  model.layers.N.self_attn.g_a_proj                  _unsafe_view     [B,d_head_kda] -> [B,1,d_head_kda]
  model.layers.N.self_attn.g_b_proj                  t                [n_h_kda*d_head_kda,d_head_kda] -> w=[n_h_kda*d_head_kda,d_head_kda] [d_head_kda,n_h_kda*d_head_kda]
  model.layers.N.self_attn.g_b_proj                  view             [B,1,d_head_kda] -> [B,d_head_kda]
  model.layers.N.self_attn.g_b_proj                  matmul           [B,d_head_kda]*[d_head_kda,n_h_kda*d_head_kda] -> w=[n_h_kda*d_head_kda,d_head_kda] [B,n_h_kda*d_head_kda]
  model.layers.N.self_attn.g_b_proj                  _unsafe_view     [B,n_h_kda*d_head_kda] -> [B,1,n_h_kda*d_head_kda]
  ...
  model.layers.N.block_sparse_moe.gate               unsqueeze        [E] -> [B,E]
  model.layers.N.block_sparse_moe.gate               add_             [B,E]*[B,E] -> [B,E]
  model.layers.N.block_sparse_moe.gate               view             [B,E] -> [B,1,E]
  model.layers.N.block_sparse_moe.gate               topk             [B,1,E] -> [B,1,2]*[B,1,2]
  model.layers.N.block_sparse_moe.gate               sum              [B,1,2] -> [B,1]
  model.layers.N.block_sparse_moe.gate               topk             [B,1] -> [B,1]*[B,1]
  model.layers.N.block_sparse_moe.gate               zeros_like       [B,1] -> [B,1]
  model.layers.N.block_sparse_moe.gate               scatter_         [B,1]*[B,1] -> [B,1]
  model.layers.N.block_sparse_moe.gate               unsqueeze        [B,1] -> [B,1,1]
  model.layers.N.block_sparse_moe.gate               expand           [B,1,1] -> [B,1,E]
  model.layers.N.block_sparse_moe.gate               view             [B,1,E] -> [B,E]
  model.layers.N.block_sparse_moe.gate               _to_copy         [B,E] -> [B,E]
  model.layers.N.block_sparse_moe.gate               bitwise_not      [B,E] -> [B,E]
  model.layers.N.block_sparse_moe.gate               masked_fill      [B,E]*[B,E] -> [B,E]
  ...
  model.layers.N.block_sparse_moe                    index            [B,d_model]*[k] -> [k,d_model]
  model.layers.N.block_sparse_moe                    slice            [k,d_model] -> [2,d_model]
  model.layers.N.block_sparse_moe.experts.N.w1       t                [d_moe,d_model] -> w=[d_moe,d_model] [d_model,d_moe]
  model.layers.N.block_sparse_moe.experts.N.w1       matmul           [2,d_model]*[d_model,d_moe] -> w=[d_moe,d_model] [2,d_moe]
  model.layers.N.block_sparse_moe.experts.N.act_fn   silu             [2,d_moe] -> [2,d_moe]
  model.layers.N.block_sparse_moe.experts.N.w3       t                [d_moe,d_model] -> w=[d_moe,d_model] [d_model,d_moe]
  model.layers.N.block_sparse_moe.experts.N.w3       matmul           [2,d_model]*[d_model,d_moe] -> w=[d_moe,d_model] [2,d_moe]
  model.layers.N.block_sparse_moe.experts.0          elementwise_mul  [2,d_moe]*[2,d_moe] -> [2,d_moe]
  model.layers.N.block_sparse_moe.experts.N.w2       t                [d_model,d_moe] -> w=[d_model,d_moe] [d_moe,d_model]
  model.layers.N.block_sparse_moe.experts.N.w2       matmul           [2,d_moe]*[d_moe,d_model] -> w=[d_model,d_moe] [2,d_model]
  model.layers.N.block_sparse_moe.experts.1          elementwise_mul  [2,d_moe]*[2,d_moe] -> [2,d_moe]
  model.layers.N.block_sparse_moe.experts.2          elementwise_mul  [2,d_moe]*[2,d_moe] -> [2,d_moe]
  model.layers.N.block_sparse_moe.experts.3          elementwise_mul  [2,d_moe]*[2,d_moe] -> [2,d_moe]
  model.layers.N.block_sparse_moe                    concat           [2,d_model]*[2,d_model]*[2,d_model]*[2,d_model] -> [k,d_model]
  model.layers.N.block_sparse_moe                    empty_like       [k,d_model] -> [k,d_model]
  model.layers.N.block_sparse_moe                    index_put_       [k,d_model]*[k]*[k,d_model] -> [k,d_model]
  model.layers.N.block_sparse_moe                    view             [k,d_model] -> [B,k,d_model]
  model.layers.N.block_sparse_moe                    _to_copy         [B,k,d_model] -> [B,k,d_model]
  model.layers.N.block_sparse_moe                    unsqueeze        [B,k] -> [B,k,1]
  model.layers.N.block_sparse_moe                    mul_             [B,k,d_model]*[B,k,1] -> [B,k,d_model]
  model.layers.N.block_sparse_moe                    sum              [B,k,d_model] -> [B,d_model]
  model.layers.N.block_sparse_moe                    _to_copy         [B,d_model] -> [B,d_model]
  ...
  model.layers.N.block_sparse_moe.shared_experts.gate_proj matmul           [B,d_model]*[d_model,d_moe] -> w=[d_moe,d_model] [B,d_moe]
  model.layers.N.block_sparse_moe.shared_experts.gate_proj _unsafe_view     [B,d_moe] -> [B,1,d_moe]
  model.layers.N.block_sparse_moe.shared_experts.act_fn silu             [B,1,d_moe] -> [B,1,d_moe]
  model.layers.N.block_sparse_moe.shared_experts.up_proj t                [d_moe,d_model] -> w=[d_moe,d_model] [d_model,d_moe]
  model.layers.N.block_sparse_moe.shared_experts.up_proj view             [B,1,d_model] -> [B,d_model]
  model.layers.N.block_sparse_moe.shared_experts.up_proj matmul           [B,d_model]*[d_model,d_moe] -> w=[d_moe,d_model] [B,d_moe]
  model.layers.N.block_sparse_moe.shared_experts.up_proj _unsafe_view     [B,d_moe] -> [B,1,d_moe]
  model.layers.N.block_sparse_moe.shared_experts     elementwise_mul  [B,1,d_moe]*[B,1,d_moe] -> [B,1,d_moe]
  model.layers.N.block_sparse_moe.shared_experts.down_proj t                [d_model,d_moe] -> w=[d_model,d_moe] [d_moe,d_model]
  model.layers.N.block_sparse_moe.shared_experts.down_proj view             [B,1,d_moe] -> [B,d_moe]
  model.layers.N.block_sparse_moe.shared_experts.down_proj matmul           [B,d_moe]*[d_moe,d_model] -> w=[d_model,d_moe] [B,d_model]
