...
# 배치 B — 대상 모델: Qwen__Qwen3.5-397B-A17B, Qwen__Qwen3.5-4B, allenai__OLMoE-1B-7B-0924


# ============================================================
# 모델: Qwen__Qwen3.5-397B-A17B
# ============================================================

# 리뷰 패킷 — Qwen/Qwen3.5-397B-A17B

> 이 문서는 **자기완결적**입니다. 판단에 필요한 것은 전부 아래에 있습니다.
> revision `8472618112abcbd45acbcdc58436aff4233c23f7` / 트레이스 seq_len(T) = 17
...
| KV cache 크기 | 2·n_kv·d_head = 2·2·256 = 1024 elems / token / layer; 15 attention layer(s) ⇒ 15360 / token |
...
| 이 모듈 스코프의 심볼 | 342,030 | 33.51% |
| 이름 없음 (정수 유지) | 137,916 | 13.51% |
| 스코프 없는 심볼 | 55,623 | 5.45% |
| 이 모듈 스코프의 유도식 | 50,373 | 4.94% |
| 휴리스틱: 심볼의 배수 | 3,780 | 0.37% |
| 같은 shape에서 이미 쓴 심볼 재사용 | 1,120 | 0.11% |

등록된 규칙 **877,789축**, 약한 근거 1,120축, 휴리스틱 **3,780축 (0.37%)**, 이름 없음 137,916축.
...
```
  ...
  model.layers.N.linear_attn                         expand           [B,T,n_h_lin_k,1,d_head_lin_k] -> [B,T,n_h_lin_k,d_conv_lin,d_head_lin_k]
  model.layers.N.linear_attn                         clone            [B,T,n_h_lin_k,d_conv_lin,d_head_lin_k] -> [B,T,n_h_lin_k,d_conv_lin,d_head_lin_k]
  model.layers.N.linear_attn                         view             [B,T,n_h_lin_k,d_conv_lin,d_head_lin_k] -> [B,T,n_h_lin_v,d_head_lin_k]
...
```
  ...
  model.layers.N.linear_attn                         expand           [B,1,n_h_lin_k,1,d_head_lin_k] -> [B,1,n_h_lin_k,d_conv_lin,d_head_lin_k]
  model.layers.N.linear_attn                         clone            [B,1,n_h_lin_k,d_conv_lin,d_head_lin_k] -> [B,1,n_h_lin_k,d_conv_lin,d_head_lin_k]
  model.layers.N.linear_attn                         view             [B,1,n_h_lin_k,d_conv_lin,d_head_lin_k] -> [B,1,n_h_lin_v,d_head_lin_k]
...
shape 축 **527,825개**를 렌더하면서 어떤 근거로 이름을 붙였는지의 내역이다. 위쪽 네 줄은 `rules/`에 **등록된 규칙**이 답을 준 경우이고, `휴리스틱`으로 시작하는 줄은 등록된 규칙이 없어 **산술적으로 맞는 이름을 지어낸** 경우다. 후자는 이번 트레이스의 seq_len에서만 참일 수 있으므로 그대로 신뢰하면 안 되고, `02-new-module-handling.md` Tier 2로 확인해 규칙으로 승격시켜야 한다.

| 근거 | 축 수 | 비율 |
|---|---:|---:|
| 런타임 축 (B/T/1) | 228,065 | 43.21% |
| 이 모듈 스코프의 심볼 | 175,579 | 33.26% |
| 이름 없음 (정수 유지) | 71,543 | 13.55% |
| 이 모듈 스코프의 유도식 | 24,793 | 4.70% |
| 스코프 없는 심볼 | 24,219 | 4.59% |
| 휴리스틱: 심볼의 배수 | 2,064 | 0.39% |
| 같은 shape에서 이미 쓴 심볼 재사용 | 890 | 0.17% |
| 휴리스틱: 심볼+1 | 672 | 0.13% |

등록된 규칙 **452,656축**, 약한 근거 890축, 휴리스틱 **2,736축 (0.52%)**, 이름 없음 71,543축.
...
| C6 | PASS | hidden_size=2560 (heuristic check, 1048 flagged) |
...
| C14 | PASS | used=17 >= required=16 |
| C15 | PASS | all discovered entrypoints traced |
| C16 | INFO | 21454 unmapped rows, 29 distinct raw ops: ['aten._to_copy.default', 'aten._unsafe_view.default', ... |
| C17 | PASS | 유도 상수 전부 설명됨, 구조 라이브러리에 등재됨 |
...
C14  PASS   used=17 >= required=16
C15  PASS   all discovered entrypoints traced
C16  INFO   21454 unmapped rows, 29 distinct raw ops: ['aten._to_copy.default', 'aten._unsafe_view.default', 'aten.alias.default', 'aten.arange.default', 'aten.clone.default', 'aten.constant_pad_nd.default', 'aten.copy_.default', 'aten.expand.default', 'aten.eye.default', 'aten.le.Tensor']
C17  PASS   유도 상수 전부 설명됨, 구조 라이브러리에 등재됨
...
# ============================================================
# 모델: allenai__OLMoE-1B-7B-0924
# ============================================================

# 리뷰 패킷 — allenai/OLMoE-1B-7B-0924

> 이 문서는 **자기완결적**입니다. 판단에 필요한 것은 전부 아래에 있습니다.
> revision `6d84c48581ece794365f2b8e9cfb043c68ade9c5` / 트레이스 seq_len(T) = 17
> 라이브러리: torch 2.13.0+cpu, transformers 5.14.1
...
# Model Summary -- allenai/OLMoE-1B-7B-0924
...
| # | 항목 | 값 |
|---|---|---|
| 1 | SCALE | 6.92B total, 1.28B active (18.5% active)  _(active = 토큰 1개 forward가 실제로 거치는 파라미터. embedding과 lm_head 포함 — 벤더 발표치는 본체만 세는 경우가 있어 다를 수 있음)_ |
| 2 | Context (tokens) | 4,096  _(config max_position_embeddings)_ |
| 3 | DATE | 2024-07-20  _(HF repo 생성일 — 대략적 출시 시점, 정확한 발표일과 다를 수 있음)_ |
| 4 | DECODER TYPE | Sparse MoE |
| 5 | Attention | MHA |
| 6 | LAYER MIX | 16× MHA  (FFN: 16× MoE) |
| 7 | KV CACHE / TOKEN (BF16) | 128.0 KiB (Moderate) |
| 8 | KEY DETAIL | MHA attention; Sparse MoE (E=64, top-8, topk-then-softmax routing) |
| 9 | Related concepts | RMSNorm, RoPE, MHA, MoE, topk-softmax routing, QK-Norm |
...
|---|---|
| 모델 타입 (config) | `olmoe` |
| attention | MHA — 16 heads (no GQA/MQA), d_head=128 |
| attention 커널 | eager (explicit softmax) |
| 위치 인코딩 | RoPE (θ=10000.0) |
| FFN | MoE — 64 routed experts, top-8, expert intermediate 1024, SwiGLU (silu·gate) [grouped_mm] |
| 정규화 | RMSNorm |
| tie embeddings | False |
| decode 방식 | autoregressive, 1 token/step, reuses KV cache (prefill builds it) |
| KV cache 크기 | 2·n_kv·d_head = 2·16·128 = 4096 elems / token / layer; 16 attention layer(s) ⇒ 65536 / token |
...
shape 축 **42,852개**를 렌더하면서 어떤 근거로 이름을 붙였는지의 내역이다. 위쪽 네 줄은 `rules/`에 **등록된 규칙**이 답을 준 경우이고, `휴리스틱`으로 시작하는 줄은 등록된 규칙이 없어 **산술적으로 맞는 이름을 지어낸** 경우다. 후자는 이번 트레이스의 seq_len에서만 참일 수 있으므로 그대로 신뢰하면 안 되고, `02-new-module-handling.md` Tier 2로 확인해 규칙으로 승격시켜야 한다.

| 근거 | 축 수 | 비율 |
|---|---:|---:|
| 런타임 축 (B/T/1) | 13,849 | 32.32% |
| 스코프 없는 심볼 | 10,701 | 24.97% |
| 이 모듈 스코프의 심볼 | 10,521 | 24.55% |
| 이 모듈 스코프의 유도식 | 6,857 | 16.00% |
| 같은 shape에서 이미 쓴 심볼 재사용 | 860 | 2.01% |
| 이름 없음 (정수 유지) | 64 | 0.15% |

등록된 규칙 **41,928축**, 약한 근거 860축, 휴리스틱 **0축 (0.0%)**, 이름 없음 64축.
...
| C6 | PASS | hidden_size=2048 (heuristic check, 1184 flagged) |
| C7 | PASS | MHA (kv_heads == heads, not GQA) |
| C8 | WARN | MoE trace-verified [router_dim(E=64):ok, top_k(8):ok, expert_weight:grouped]; routed-token count ... |
...
| C14 | PASS | used=17 >= required=16 |
| C15 | PASS | all discovered entrypoints traced |
| C16 | INFO | 1255 unmapped rows, 24 distinct raw ops: ['aten._to_copy.default', 'aten._unsafe_view.default', '... |
| C17 | PASS | 유도 상수 전부 설명됨, 구조 라이브러리에 등재됨 |
...
| 구분 | 소스 | 역할 |
|---|---|---|
| config (1차) | HF `allenai/OLMoE-1B-7B-0924` config.json @ `6d84c48581ece794365f2b8e9cfb043c68ade9c5` (sha256 `8e163370d5e2…`) | 심볼 값의 출처 |
| modeling code (1차) | transformers 5.14.1 공식 modeling forward (meta device) | op·shape·dependency 캡처 |
| trace (1차) | dispatch(ATen) 레벨, seq_len(T)=17 | 표·그래프 생성 근거 |
...
C14  PASS   used=17 >= required=16
C15  PASS   all discovered entrypoints traced
C16  INFO   1255 unmapped rows, 24 distinct raw ops: ['aten._to_copy.default', 'aten._unsafe_view.default', 'aten.alias.default', 'aten.arange.default', 'aten.clamp_.default', 'aten.clone.default', 'aten.empty_like.default', 'aten.expand.default', 'aten.floor_divide.default', 'aten.ge.Scalar']
C17  PASS   유도 상수 전부 설명됨, 구조 라이브러리에 등재됨
